...
async function renderHomePrayerTimes() {
    ...
    navigator.geolocation.getCurrentPosition(async position => {
        try {
            ...
            const labels = [['Fajr','الفجر'], ...(prayerDisplaySettings.showSunrise === false ? [] : [['Sunrise','الشروق']]), ['Dhuhr','الظهر'],['Asr','العصر'],['Maghrib','المغرب'],['Isha','العشاء']];
            list.innerHTML = `<div class="prayer-grid">${labels.map(([key, label]) => `<div class="prayer-time-item"><strong>${label}</strong><span dir="rtl">${escapeHtml(toArabic12(times[key]))}</span></div>`).join('')}</div>`;
