...
function startAppAfterAccess() {
    ...
    if ('serviceWorker' in navigator) navigator.serviceWorker.register(`./sw-v61.js?v=61`, { updateViaCache: 'none' }).catch(() => {});
}
...
async function initializeOfflineFirstApp() {
        ...
        window.moveKhatmaToTopic = async function(key) {
            ...
            overlay.querySelector('#topicMoveSave').onclick = async () => {
                ...
                try { await requireAdminSession(); await db.ref(`khatmas/${key}/customTopic`).set(topic); record.customTopic = topic; await OfflineStore.putKhatma(key, record); overlay.remove(); filterKhatmas(); if (typeof renderAdminTopicBrowser === 'function') renderAdminTopicBrowser(); } catch (e) { save.disabled = false; status.textContent = `تعذر الحفظ: ${e.code || e.message || 'تحقق من صلاحيات Firebase.'}`; }
            };
        };
