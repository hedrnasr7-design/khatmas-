...
function renderDateBar() {
    ...
    const dateMarkup = `<div>${formatted.weekday}</div><span class="date-separator">/</span><div>${formatted.gregorian}</div><span class="date-separator">/</span><div>${formatted.hijri}</div>`;
    const extra = String(settings.extraText || '').trim();
    const copyMarkup = `<span class="date-copy">${dateMarkup}${extra ? `<span class="date-separator">•</span><span class="date-extra-text">${escapeHtml(extra)}</span>` : ''}</span>`;
    track.innerHTML = copyMarkup;
    const firstCopy = track.querySelector('.date-copy');
    const copyWidth = Math.max(1, firstCopy?.getBoundingClientRect().width || track.scrollWidth);
    const barWidth = Math.max(1, bar.getBoundingClientRect().width);
    const travelDistance = barWidth + copyWidth;
    const effectiveSpeed = Math.max(5, Math.min(120, Number(settings.speed) || 12));
    const duration = Math.max(4, Math.min(120, travelDistance / (effectiveSpeed * 10)));
    bar.style.setProperty('--date-start', `${barWidth}px`);
    bar.style.setProperty('--date-end', `-${copyWidth}px`);
