...
async function uploadImagesToFirebaseStorage(files, folder, status) {
    ...
    if (!firebase.storage) throw new Error('Firebase Storage غير مفعّل في المشروع.');
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        const path = `public/${folder}/${Date.now()}_${i}_${safeName}`;
        // صور الهاتف قد تكون كبيرة جدًا؛ ضغطها قبل Storage يمنع retry-limit-exceeded.
        const compressedDataUrl = await compressDailyMessageImage(file);
        const compressedBlob = await (await fetch(compressedDataUrl)).blob();
        let snapshot = null;
        let lastError = null;
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                if (status) status.textContent = `جارٍ رفع الصورة ${i + 1} من ${files.length} (المحاولة ${attempt + 1})...`;
                snapshot = await storage.ref(path).put(compressedBlob, { contentType: 'image/jpeg', cacheControl: 'public,max-age=31536000' });
                break;
            } catch (error) {
                lastError = error;
        ...
        if (!snapshot) throw lastError || new Error('تعذر رفع الصورة.');
...
window.saveQuranBenefitsSettings = async () => {
    ...
    try {
        ...
        if (status) status.textContent = `فشل الحفظ: ${error?.code || error?.message || 'تحقق من تفعيل Firebase Storage وقواعده.'}`;
    }
};
...
window.saveNamesOfAllahSettings = async () => {
    ...
    try {
        ...
        if (status) status.textContent = `تم رفع وحفظ ${toArabicNum(String(uploaded.length))} صورة لجميع المستخدمين.`;
    } catch (error) { console.error('تعذر حفظ أسماء الله الحسنى:', error); if (status) status.textContent = `فشل الحفظ: ${error?.code || error?.message || 'تحقق من تفعيل Firebase Storage وقواعده.'}`; }
};
