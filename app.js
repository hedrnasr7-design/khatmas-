...
function startAppAfterAccess() {
    ...
    if ('serviceWorker' in navigator) navigator.serviceWorker.register(`./sw-v79.js?v=79`, { updateViaCache: 'none' }).catch(() => {});
}
...
async function syncDataFromNetwork() {
            // تحميل موثوق: إذا فشل الفهرس أو كانت نسخة الهاتف تالفة ننتقل مباشرة إلى Firebase.
            if (syncInProgress) return;
            syncInProgress = true;
            lastSyncAttempt = Date.now();
            const urls = [
                `./khatmas-index.json?v=76`,
                `${DATABASE_URL}/khatmas.json`
            ];
            let lastError = null;
            try {
                for (const url of urls) {
                    try {
                        ...
                        const response = await fetch(url, {
                            method: 'GET',
                            cache: 'no-store',
                            headers: url.includes('firebaseio.com') ? { 'X-Firebase-ETag': 'true' } : {},
                            signal: controller.signal
                        });
                        clearTimeout(timeout);
                        if (!response.ok) throw new Error(`تعذر جلب البيانات: ${response.status}`);
                        const data = (await response.json()) || {};
                        if (!data || typeof data !== 'object' || Object.keys(data).length === 0) {
                            throw new Error('بيانات الختمات فارغة');
                        }
                        const cleanData = removeDeletedKhatmaFromData(data);
                        // نعرض البيانات أولًا؛ فشل IndexedDB لا يجب أن يخفي الختمات.
                        allKhatmas = cleanData;
                        stopLoadingProgress();
                        renderCurrentData();
                        try {
                            await OfflineStore.replaceAllKhatmas(cleanData);
                            await OfflineStore.setMeta('last-successful-sync', new Date().toISOString());
                        } catch (storageError) {
                            console.warn('تعذر حفظ نسخة محلية، لكن البيانات عُرضت:', storageError);
                        }
                        return;
                    } catch (error) {
                        lastError = error;
                        console.warn('فشل مصدر البيانات:', url, error);
                    }
                }
                throw lastError || new Error('تعذر تحميل الختمات من جميع المصادر');
            } catch (error) {
