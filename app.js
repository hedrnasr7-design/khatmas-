...
async function loadMafatihTodaySettings() { let localSaved = {}; try { localSaved = JSON.parse(localStorage.getItem('mafatih_today_settings') || '{}') || {}; } catch (error) {} let remoteSaved = {}; try { const snapshot = await db.ref('appSettings/mafatihToday').once('value'); if (snapshot.val()) remoteSaved = snapshot.val(); } catch (error) {} const saved = Number(localSaved.updatedAt || 0) >= Number(remoteSaved.updatedAt || 0) ? { ...remoteSaved, ...localSaved } : { ...localSaved, ...remoteSaved }; mafatihTodaySettings = applyMafatihTodaySettings(saved); localStorage.setItem('mafatih_today_settings', JSON.stringify(mafatihTodaySettings)); renderMafatihToday(); }
window.saveMafatihTodaySettings = async () => { const s = { ...applyMafatihTodaySettings({enabled:document.getElementById('mafatihTodayEnabled')?.checked, count:document.getElementById('mafatihTodayCount')?.value, bg:document.getElementById('mafatihTodayBg')?.value, accent:document.getElementById('mafatihTodayAccent')?.value, text:document.getElementById('mafatihTodayText')?.value, border:document.getElementById('mafatihTodayBorder')?.value, font:document.getElementById('mafatihTodayFont')?.value, size:document.getElementById('mafatihTodaySize')?.value, customItems:document.getElementById('mafatihTodayCustomItems')?.value || '', displayOverride:document.getElementById('mafatihTodayDisplayText')?.value || ''}, true), updatedAt:Date.now() }; mafatihTodaySettings = s; localStorage.setItem('mafatih_today_settings', JSON.stringify(s)); try { await db.ref('appSettings/mafatihToday').set({ ...s, updatedAt: firebase.database.ServerValue.TIMESTAMP }); } catch (error) {} await renderMafatihToday(); const status = document.getElementById('mafatihTodaySettingsStatus'); if (status) status.textContent = 'تم حفظ إعدادات أعمال اليوم لجميع المستخدمين.'; };
