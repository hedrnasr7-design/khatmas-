...
function applyUserSettings(settings = {}, persist = false) {
    ...
    if (!style) { style = document.createElement('style'); style.id = 'userSettingsRuntimeStyles'; document.head.appendChild(style); }
...
window.saveUserSettings = () => { const values = {}; Object.entries(userSettingIds).forEach(([key, id]) => { const el = document.getElementById(id); values[key] = el?.type === 'number' ? Number(el.value) : el?.value; }); applyUserSettings(values, true); };
