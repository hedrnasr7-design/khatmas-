...
window.saveDailyCardsSettings = async () => {
    ...
    try {
        ...
        await db.ref('appSettings/dailyCards').set({ ...settings, updatedAt: firebase.database.ServerValue.TIMESTAMP });
        if (status) status.textContent = `تم حفظ صور يوم ${document.getElementById('dailyCardsDay')?.selectedOptions?.[0]?.text || day}.`;
    ...
    } catch (error) { console.error(error); if (status) status.textContent = 'تعذر حفظ الصور؛ تأكد من حجمها وصلاحيات المشرف.'; }
};
...
window.saveDailyDuaVisitSettings = async () => {
    ...
    try {
        ...
        await db.ref('appSettings/dailyDuaVisit').set({ ...settings, updatedAt: firebase.database.ServerValue.TIMESTAMP });
        if (status) status.textContent = `تم حفظ صور دعاء وزيارة يوم ${document.getElementById('dailyDuaVisitDay')?.selectedOptions?.[0]?.text || day}.`;
...
window.saveNightPrayerSettings = async () => {
    ...
    try {
        ...
        await db.ref('appSettings/nightPrayer').set({ ...settings, updatedAt: firebase.database.ServerValue.TIMESTAMP });
        if (status) status.textContent = `تم حفظ صور صلاة ${document.getElementById('nightPrayerDay')?.selectedOptions?.[0]?.text || night}.`;
...
function getCurrentHijriDayNumber() {
    ...
    return Math.max(1, Math.min(30, Number(parts.find(part => part.type === 'day')?.value) || 1));
}
...
window.saveArabicMonthDaySettings = async () => {
    ...
    try {
        ...
        await db.ref('appSettings/arabicMonth').set({ ...settings, updatedAt: firebase.database.ServerValue.TIMESTAMP });
        if (status) status.textContent = `تم حفظ صور اليوم الهجري ${toArabicNum(day)}.`;
