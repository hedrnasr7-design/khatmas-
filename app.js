...
async function initializeOfflineFirstApp() {
        ...
        window.renderList = (data, view = 'all') => {
            ...
            sortedKeys.forEach(key => {
                ...
                const dragAttrs = isAdminSessionActive() ? ` public-topic-draggable draggable="true" ondragstart="topicDragStart(event, '${key}')" ondragend="topicDragEnd(event)" ontouchstart="topicTouchStart(event, '${key}')" ontouchmove="topicTouchMove(event)" ontouchend="topicTouchEnd(event)"` : '';
                htmlContent += `<div class="khatma-item${dragAttrs ? ' public-topic-draggable' : ''}"${dragAttrs}><div class="khatma-title" onclick="show('${key}')">${escapeHtml(displayTitle)}${typeLabel}</div><button onclick="event.stopPropagation(); toggleFav('${key}')" style="background:none; border:none; font-size:20px; cursor:pointer;">${isFav}</button></div>`;
            });
        ...
        function topicDragEnd(event) { event.currentTarget.classList.remove('topic-dragging'); draggedTopicKhatma = null; document.querySelectorAll('.admin-topic-card.topic-drop-over').forEach(el => el.classList.remove('topic-drop-over')); }
        ...
        async function topicDrop(event, topic) {
            event.preventDefault(); event.stopPropagation(); event.currentTarget.classList.remove('topic-drop-over');
            const key = draggedTopicKhatma || event.dataTransfer?.getData('text/plain');
            draggedTopicKhatma = null;
            if (!key || !allKhatmas[key] || String(topic).trim() === 'all') return;
            const record = allKhatmas[key];
            const previous = record.customTopic || '';
            if (previous.trim() === topic.trim()) return;
            try {
                ...
                const status = document.getElementById('adminTopicStatus');
                if (status) status.textContent = `تعذر حفظ النقل: ${error.code || error.message || 'تحقق من تسجيل دخول المشرف.'}`;
                alert(`تعذر حفظ نقل الختمة: ${error.code || error.message || 'تحقق من الصلاحيات.'}`);
            }
        }
        function topicTouchStart(event, key) {
            const target = event.currentTarget;
            clearTimeout(longPressTimer);
            longPressTimer = setTimeout(() => { draggedTopicKhatma = key; target.classList.add('topic-dragging'); target.dataset.longDragging = '1'; }, 500);
        }
        function topicTouchMove(event) {
            if (!draggedTopicKhatma) return;
            event.preventDefault();
            const touch = event.touches[0], under = document.elementFromPoint(touch.clientX, touch.clientY)?.closest('.admin-topic-card, .topic-tab');
            document.querySelectorAll('.admin-topic-card.topic-drop-over').forEach(el => el.classList.remove('topic-drop-over'));
            if (under) under.classList.add('topic-drop-over');
        }
        async function topicTouchEnd(event) {
            clearTimeout(longPressTimer);
            if (!draggedTopicKhatma) return;
            const touch = event.changedTouches[0], under = document.elementFromPoint(touch.clientX, touch.clientY)?.closest('.admin-topic-card, .topic-tab');
...
function renderTodayKhatmas() {
    ...
    const rows = getScheduleRows('').filter(row => {
        const info = row.info;
        const nextDay = (now.weekday + 1) % 7;
        const matchesToday = info.weekdays.includes(now.weekday) || (isIraqNight(now) && info.weekdays.includes(nextDay));
        return info.weekdays.length ? matchesToday : (info.times.length || info.periods.length || getScheduleOverride(row.key));
    });
    ...
    const level = Math.max(1, Math.min(100, Number(window.todayKhatmasSettings?.motionLevel) || 50));
    // السرعة المتوسطة = 105 ثانية للدورة، دون فراغ طويل بين التكرارات.
    track.style.animationDuration = `${Math.round(180 - (level - 1) * (150 / 99))}s`;
}
