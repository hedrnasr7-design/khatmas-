const firebaseConfig = {
        ...
        async function fetchKhatmaIndex(forceFresh = false) {
            const previousEtag = forceFresh ? null : await OfflineStore.getMeta('firebase-index-etag');
            const headers = { 'X-Firebase-ETag': 'true' };
            if (previousEtag) headers['If-None-Match'] = previousEtag;
            const response = await fetch(`${DATABASE_URL}/khatmaIndex.json`, { method: 'GET', headers, cache: 'no-store' });
            if (response.status === 404) return null;
            if (response.status === 304) return { notModified: true };
            if (!response.ok) throw new Error(`تعذر جلب فهرس الختمات: ${response.status}`);
            ...
            Object.keys(raw).forEach(key => {
                const item = raw[key] || {};
                index[key] = { title: item.title || 'بدون عنوان', content: '', image: item.hasImage ? '1' : '', isIndex: true };
            });
            const etag = response.headers.get('ETag');
            if (etag) await OfflineStore.setMeta('firebase-index-etag', etag);
            return index;
        }

        async function fetchLegacyKhatmas(forceFresh = false) {
            const previousEtag = forceFresh ? null : await OfflineStore.getMeta('firebase-etag');
            const headers = { 'X-Firebase-ETag': 'true' };
            if (previousEtag) headers['If-None-Match'] = previousEtag;
            const response = await fetch(`${DATABASE_URL}/khatmas.json`, { method: 'GET', headers, cache: 'no-store' });
            if (response.status === 304) return { notModified: true };
            if (!response.ok) throw new Error(`تعذر جلب الختمات القديمة: ${response.status}`);
            const data = (await response.json()) || {};
            const etag = response.headers.get('ETag');
            if (etag) await OfflineStore.setMeta('firebase-etag', etag);
            return data;
        }

        async function syncFullKhatmasForAdmin() {
            if (!navigator.onLine) return;
            let localFull = {};
            try { localFull = await OfflineStore.getAllKhatmas(); } catch (ignore) {}
            const hasFullLocalData = Object.values(localFull || {}).some(item => item && (item.content || item.image));
            if (hasFullLocalData) {
                allKhatmas = localFull;
            } else {
                const response = await fetch(`${DATABASE_URL}/khatmas.json`, { cache: 'no-store' });
                if (!response.ok) throw new Error(`تعذر تحميل بيانات الإدارة: ${response.status}`);
                allKhatmas = (await response.json()) || {};
            }
            Object.values(allKhatmas).forEach(item => { if (item) item.isIndex = false; });
            const index = {};
            Object.keys(allKhatmas).forEach(key => {
                const item = allKhatmas[key] || {};
                index[key] = { title: item.title || 'بدون عنوان', hasImage: !!(item.image && item.image.trim()) };
            });
            await db.ref('khatmaIndex').set(index);
            renderCurrentData();
        }
        ...
        async function syncDataFromNetwork() {
            ...
            try {
                let data = await fetchKhatmaIndex();
                if (data && data.notModified && Object.keys(allKhatmas).length === 0) data = await fetchKhatmaIndex(true);
                if (data === null) {
                    const currentUser = firebase.auth().currentUser;
                    const isAdmin = currentUser && (currentUser.email || '').toLowerCase() === ADMIN_EMAIL.toLowerCase();
                    if (isAdmin) {
                        await syncFullKhatmasForAdmin();
                    } else if (Object.keys(allKhatmas).length === 0) {
                        try {
                            let legacyData = await fetchLegacyKhatmas();
                            if (legacyData.notModified && Object.keys(allKhatmas).length === 0) legacyData = await fetchLegacyKhatmas(true);
                            if (!legacyData.notModified) {
                                allKhatmas = legacyData;
                                renderCurrentData();
                                try { await OfflineStore.replaceAllKhatmas(legacyData); } catch (cacheError) { console.warn('تعذر حفظ النسخة المحلية، لكن البيانات ستظل معروضة:', cacheError); }
                            } else {
                                renderCurrentData();
                            }
                        } catch (legacyError) {
                            console.warn('الفهرس غير موجود وتعذر تحميل المسار القديم:', legacyError);
                            document.getElementById('listContainer').innerHTML = '<div class="loading-box">تعذر تحميل الختمات حاليًا. تأكد من عودة قاعدة البيانات للعمل أو سجّل دخول المشرف لإنشاء الفهرس.</div>';
                        }
                    }
                } else {
                    if (data && !data.notModified) {
                        allKhatmas = data;
                        renderCurrentData();
                        try { await OfflineStore.replaceAllKhatmas(data); } catch (cacheError) { console.warn('تعذر حفظ الفهرس محليًا، لكن البيانات ستظل معروضة:', cacheError); }
                        localStorage.removeItem('offline_khatmas_perfect');
                    } else {
                        renderCurrentData();
                    }
                ...
                console.warn('يتعذر تحميل الفهرس؛ تُستخدم النسخة المحلية:', error);
                stopLoadingProgress();
        ...
        document.getElementById('adminTrigger').onclick = async () => {
            ...
            try {
                ...
                try {
                    ...
                    console.warn('تعذر تحميل بيانات الإدارة كاملة:', loadError);
                    renderAdminLists(allKhatmas);
                }
        ...
        window.show = async (key) => {
            if (!allKhatmas[key]) return;
            let item = allKhatmas[key];
            if (item.isIndex || (!item.content && item.image === '1')) {
                document.getElementById('displayTitle').innerText = item.title || 'جاري التحميل...';
                document.getElementById('displayContent').innerHTML = '<p>جاري تحميل محتوى الختمة...</p>';
                document.getElementById('modalOverlay').style.display = 'flex';
                try {
                    const response = await fetch(`${DATABASE_URL}/khatmas/${encodeURIComponent(key)}.json`, { cache: 'no-store' });
                    if (!response.ok) throw new Error('تعذر تحميل الختمة');
                    item = (await response.json()) || item;
                    allKhatmas[key] = item;
                } catch (error) {
                    document.getElementById('displayContent').innerHTML = '<p>تعذر تحميل المحتوى. تحقق من الاتصال بالإنترنت.</p>';
                    return;
                }
            }
            document.getElementById('displayTitle').innerText = item.title || 'بدون عنوان';
            const displayContent = document.getElementById('displayContent');
            let htmlOutput = '';
            if (item.image && item.image.trim() !== '' && item.image !== '1') {
                htmlOutput += `<div style="text-align: center; margin-bottom: 15px;"><img src="${item.image}" alt="${item.title}" style="max-width: 100%; height: auto; border-radius: 8px;"></div>`;
            }
            if (item.content && item.content.trim() !== '') htmlOutput += `<div>${item.content.replace(/\n/g, '<br>')}</div>`;
