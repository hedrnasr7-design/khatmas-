...
// أبرز المناسبات الشيعية الثابتة حسب اليوم والشهر الهجري.
// بعض المناسبات لها أكثر من قول تاريخي؛ لذلك تظهر بصياغة "بحسب رواية" عند الحاجة.
const shiaOccasionsByMonthDay = {
    ...
    '1-10': ['عاشوراء — ذكرى شهادة الإمام الحسين (عليه السلام) وأهل بيته وأصحابه'],
    ...
    '2-7': ['ذكرى شهادة الإمام الحسن المجتبى (عليه السلام) — بحسب رواية'],
    ...
    '4-10': ['ذكرى ولادة الإمام الحسن العسكري (عليه السلام) — بحسب رواية'],
...
function getShiaHijriDate(now = getIraqNow(), hijriOffset = 0) {
    // التقويم الحسابي المدني يعطي تاريخًا ثابتًا قابلًا لإعادة الإنتاج،
    // وهو أقرب إلى تقاويم المناسبات الشيعية من Umm al-Qura في هذه الخانة.
    const shifted = new Date(Date.UTC(now.year, now.month - 1, now.day));
    shifted.setUTCDate(shifted.getUTCDate() + (Number(hijriOffset) || 0));
    ...
    const julianDay = Math.floor(365.25 * (year + 4716)) + Math.floor(30.6001 * (month + 1)) + day + correction - 1524.5;
    const lunarBase = julianDay - 1948440 + 10632;
    const cycle = Math.floor((lunarBase - 1) / 10631);
    const remainder = lunarBase - 10631 * cycle + 354;
    const cycleYear = Math.floor((10985 - remainder) / 5316) * Math.floor((50 * remainder) / 17719) + Math.floor(remainder / 5670) * Math.floor((43 * remainder) / 15238);
    const monthDay = remainder - Math.floor((30 - cycleYear) / 15) * Math.floor((17719 * cycleYear) / 50) - Math.floor(cycleYear / 16) * Math.floor((15238 * cycleYear) / 43) + 29;
    const hijriMonth = Math.floor((24 * monthDay) / 709);
    return { day: monthDay - Math.floor((709 * hijriMonth) / 24), month: hijriMonth, year: 30 * cycle + cycleYear - 30 };
}
