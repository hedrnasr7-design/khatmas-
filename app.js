...
function getIraqDayLabel(now = getIraqNow()) {
    if (!isIraqNight(now)) return `اليوم: ${scheduleWeekdays[now.weekday]}`;
    return `الليلة: ليلة ${scheduleWeekdays[(now.weekday + 1) % 7]}`;
}
...
function formatArabicNumericDate(now, hijriOffset = 0) {
    ...
    const gregorian = `${toArabicNum(String(now.day).padStart(2, '0'))}/${toArabicNum(String(now.month).padStart(2, '0'))}/${toArabicNum(String(now.year))}`;
    const hijri = `${toArabicNum(getPart('day').padStart(2, '0'))}/${toArabicNum(getPart('month').padStart(2, '0'))}/${toArabicNum(getPart('year'))}`;
...
function renderScheduleResults(rows) {
    ...
    box.innerHTML = `<table class="date-schedule-table"><thead><tr><th>الرقم</th><th>العنوان</th><th>اليوم المستخرج</th><th>الوقت المستخرج</th><th>النوع</th></tr></thead><tbody>${rows.map(row => {
        ...
        const times = [...i.times.map(time => time.text), ...i.periods.map(period => period.label), ...i.timePhrases].filter((value, index, list) => list.indexOf(value) === index).join('، ') || 'غير محدد';
...
function findNextScheduledKhatma(now = getIraqNow()) {
    ...
    const rows = getScheduleRows('').filter(row => row.info.times.length || row.info.periods.length);
    const candidates = [];
    rows.forEach(row => {
        row.info.times.forEach(time => {
...
function renderDateBar() {
    ...
    if (live) {
        live.textContent = `${dateText} • ${formatted.gregorian} • هـ ${formatted.hijri} • ${formatIraqClock(iraqNow)}`;
...
window.saveDateSettings = () => {
    ...
    localStorage.setItem('date_display_settings', JSON.stringify({ mode, customDate, hijriOffset, font, bgColor, textColor, autoSchedule, extraText, speed }));
    renderDateBar();
    const status = document.getElementById('dateSettingsStatus');
