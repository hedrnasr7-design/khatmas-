...
async function initializeOfflineFirstApp() {
        ...
        window.renderAdminTopicBrowser = () => {
            ...
            browser.innerHTML = groups.size ? [...groups.entries()].sort((a,b) => a[0].localeCompare(b[0], 'ar')).map(([topic, rows]) => `<div style="background:#fff; border:1px solid #1b4332; border-radius:7px; margin:6px 0; padding:7px;"><strong style="color:#1b4332;">${escapeHtml(topic)} (${rows.length})</strong>${rows.sort((a,b) => extractNumber(a[1].title)-extractNumber(b[1].title)).map(([key, record]) => { const image = record.image && String(record.image).trim(); const label = image ? (getImageDisplayTitles()[key] || record.title || 'بدون عنوان') : (record.title || 'بدون عنوان'); return `<div style="display:flex; justify-content:space-between; gap:6px; border-top:1px solid #eee; padding:5px 0; font-size:13px;"><span>${escapeHtml(label)}</span><button type="button" onclick="edit('${key}')" style="cursor:pointer;">تعديل</button></div>`; }).join('')}</div>`).join('') : '<p style="color:#666;">لا توجد نتائج مطابقة.</p>';
        };
