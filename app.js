...
async function initializeOfflineFirstApp() {
        ...
        function topicDragEnd(event) { event.currentTarget.classList.remove('topic-dragging'); draggedTopicKhatma = null; document.querySelectorAll('.admin-topic-card.topic-drop-over,.topic-tab.topic-drop-over').forEach(el => el.classList.remove('topic-drop-over')); }
        ...
        async function topicDrop(event, topic) {
            event.preventDefault();
            if (!isAdminSessionActive()) return; event.stopPropagation(); event.currentTarget.classList.remove('topic-drop-over');
        ...
        function topicTouchStart(event, key) {
            ...
            longPressTimer = setTimeout(() => { draggedTopicKhatma = key; target.classList.add('topic-dragging'); target.dataset.longDragging = '1'; if (navigator.vibrate) navigator.vibrate(35); }, 550);
        }
