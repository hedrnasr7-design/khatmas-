const firebaseConfig = {
        ...
        function startRealtimeKhatmasListener() {
            ...
            khatmasRef.on('value', (snapshot) => {
                const data = snapshot.val() || {};
                const receivedCount = Object.keys(data).length;

                // تصل هنا الإضافات والتعديلات تلقائيًا من Firebase.
                allKhatmas = data;
                lastSyncAttempt = Date.now();
                renderCurrentData();

                // حفظ آخر نسخة محليًا دون تعطيل عرض التطبيق.
                OfflineStore.replaceAllKhatmas(data)
                    .then(() => OfflineStore.setMeta(
                        'last-successful-sync',
                        new Date().toISOString()
                    ...
                    .catch((error) => {
                        console.warn('تعذر حفظ التحديث محليًا:', error);
                    });

                console.log(`وصل تحديث Firebase تلقائيًا: ${receivedCount} ختمة`);
            }, (error) => {
                realtimeListenerStarted = false;
                console.warn('تعذر الاستماع إلى تحديثات Firebase:', error);
            });
        }
