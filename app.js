...
function renderTodayKhatmas() {
    ...
    const dailyPattern = /(?:ختمة\s+يومية|يوم(?:يًا|يا)|كل\s+يوم|كل\s+ليلة|(?:يوم|ليلة)\s*(?:الاحد|الأحد|الاثنين|الإثنين|الثلاثاء|الاربعاء|الأربعاء|الخميس|الجمعة|السبت)|(?:خاصة|خاص|مخصصة)\s*(?:ب|في)?\s*(?:يوم|ليلة)\s*(?:الاحد|الأحد|الاثنين|الإثنين|الثلاثاء|الاربعاء|الأربعاء|الخميس|الجمعة|السبت)|(?:قبل|بعد)\s+صلاة\s+(?:الصبح|الفجر|الظهر|الظهرين|العصر|المغرب|العشاء)|(?:عند|وقت)\s+(?:السحر|الفجر|الزوال|الغروب|شروق\s+الشمس|الشروق|الضحى)|(?:صباحًا|صباحا|في\s+الصباح|مساءً|مساءا|في\s+المساء)|(?:عصر|ظهر|مغرب|عشاء)\s+يوم\s+(?:الاحد|الأحد|الاثنين|الإثنين|الثلاثاء|الاربعاء|الأربعاء|الخميس|الجمعة|السبت))/i;
    const now = getIraqNow();
    const rows = getScheduleRows('').filter(row => {
        const source = `${row.info.title}\n${row.info.content}`;
        if (!dailyPattern.test(source)) return false;
        // اليوم المحدد يجب أن يطابق اليوم العراقي الحالي.
        if (row.info.weekdays.length && !row.info.weekdays.includes(now.weekday)) return false;
        const minuteNow = now.hour * 60 + now.minute;
        const override = getScheduleOverride(row.key);
        const exactTimes = override ? [override] : row.info.times;
        if (exactTimes.length) {
            // نعرض الختمة لمدة ساعة من وقتها حتى لا تختفي بسبب اختلاف الدقائق.
            return exactTimes.some(time => {
                const start = time.hour * 60 + time.minute;
                return minuteNow >= start && minuteNow < start + 60;
            });
        }
        if (row.info.periods.length) {
            return row.info.periods.some(period => minuteNow >= period.startMinute && minuteNow < period.endMinute);
        }
        // «كل يوم» أو «يوم الخميس» بلا ساعة محددة: صالح طوال ذلك اليوم.
        return true;
    });
    ...
    const value = labels.length ? labels : ['لا توجد ختمات حاليا'];
