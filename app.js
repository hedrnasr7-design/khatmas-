...
function startAppAfterAccess() {
    ...
    if ('serviceWorker' in navigator) navigator.serviceWorker.register(`./sw-v62.js?v=62`, { updateViaCache: 'none' }).catch(() => {});
}
        ...
        window.moveKhatmaToTopic = async function(key) {
            ...
            overlay.querySelector('#topicMoveSave').onclick = async () => {
                ...
                try {
                    ...
                    status.textContent = `لم يكتمل الحفظ: ${e.code || e.message || 'تحقق من الاتصال وصلاحيات Firebase.'}`;
                }
            };
        };
