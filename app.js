const firebaseConfig = {
        ...
        async function fetchKhatmaIndex() {
            const previousEtag = await OfflineStore.getMeta('firebase-index-etag');
        ...
        async function syncDataFromNetwork() {
            ...
            try {
                const data = await fetchKhatmaIndex();
                if (data === null) {
                    ...
                    if (isAdmin) {
                        ...
                        document.getElementById('listContainer').innerHTML = '<div class="loading-box">يجري تجهيز فهرس الختمات بواسطة المشرف. حاول لاحقًا.</div>';
                    }
                } else {
