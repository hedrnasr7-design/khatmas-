...
function renderTodayKhatmas() {
    ...
    // مصدر موحد: بحث عام في كل الختمات، ثم استخراج الوقت وترتيب النتائج.
    ...
    // نسختان متطابقتان تضمنان دورة متصلة بلا فراغ.
    ...
    // السرعة هنا بالبكسل/ثانية، لا بمدة ثابتة؛ لذلك لا يسرع النص
    // عندما يزداد عدد الختمات وطول الشريط.
    const pixelsPerSecond = 20 + level * 1.0;
    const cyclePixels = Math.max(track.scrollWidth / 2, 1);
    const duration = Math.max(30, Math.round(cyclePixels / pixelsPerSecond));
