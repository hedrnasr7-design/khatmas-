...
function isIraqNight(now = getIraqNow()) {
    return now.hour * 60 + now.minute >= getApproximateBaghdadSunsetHour(now.month);
}
...
function renderDateBar() {
    ...
    // المطلوب في الخانة: اسم اليوم فقط وبجانبه الساعة بنظام ١٢ ساعة.
    const dateText = getIraqDayLabel(iraqNow);
    if (live) {
        live.textContent = `${dateText} • ${formatIraqClock(iraqNow)}`;
