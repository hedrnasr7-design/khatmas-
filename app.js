...
function applyMafatihTodaySettings(settings = {}, persist = false) {
    mafatihTodaySettings = { enabled: true, count: 5, bg: '#fffdf0', accent: '#d4af37', text: '#1b4332', border: '#1b4332', font: 'Amiri, serif', size: 18, customItems: '', displayOverride: '', ...(settings || {}) };
...
function renderMafatihToday() {
    ...
    const text = override || custom;
    box.innerHTML = text ? text.split(/\n+/).filter(Boolean).map(line => `<div>${escapeHtml(line)}</div>`).join('') : '<div style="text-align:center;padding:18px;">لا توجد أعمال اليوم مضافة حاليًا.</div>';
}
...
async function loadMafatihTodaySettings() {
    ...
    applyMafatihTodaySettings(saved); renderMafatihToday(); requestHomePrayerLocation();
}
...
if (typeof loadPrayerDisplaySettings === 'function') loadPrayerDisplaySettings();
...
function requestHomePrayerLocation() {
    const box = document.getElementById('mafatihPrayerBox');
    if (box) box.style.display = 'none';
    return; // تم إلغاء مواقيت الصلاة نهائيًا لتجنب تعليق التطبيق أو طلب الموقع.
    /*
    if (!navigator.geolocation) { renderHomePrayerTimes(); return; }
    let saved = null; try { saved = JSON.parse(localStorage.getItem('home_prayer_location') || 'null'); } catch (error) {}
    if (saved?.latitude && saved?.longitude) homePrayerLocation = saved;
    renderHomePrayerTimes();
    navigator.geolocation.getCurrentPosition(position => {
        const latitude = position.coords.latitude, longitude = position.coords.longitude;
        const timezoneOffset = -new Date().getTimezoneOffset() / 60;
        homePrayerLocation = { latitude, longitude, timezoneOffset, name:'موقعك الحالي' };
        localStorage.setItem('home_prayer_location', JSON.stringify(homePrayerLocation));
        renderHomePrayerTimes();
    }, () => renderHomePrayerTimes(), { enableHighAccuracy:false, timeout:10000, maximumAge:86400000 });
    */
}
