...
async function renderHomePrayerTimes() {
    ...
    try {
        const times = getBaghdadPrayerTimes(getIraqNow());
        const toArabic12 = minutes => {
            const total = Math.round(Number(minutes));
            const hour24 = Math.floor(total / 60) % 24;
            const minute = total % 60;
            const hour12 = hour24 % 12 || 12;
            return `${toArabicNum(String(hour12))}:${toArabicNum(String(minute).padStart(2, '0'))} ${hour24 >= 12 ? 'م' : 'ص'}`;
        };
        const labels = [['fajr','الفجر'],['sunrise','الشروق'],['dhuhr','الظهر'],['asr','العصر'],['maghrib','المغرب'],['isha','العشاء']];
        list.innerHTML = `<div class="prayer-grid">${labels.map(([key,label]) => `<div class="prayer-time-item"><strong>${label}</strong><span dir="rtl">${toArabic12(times[key])}</span></div>`).join('')}</div>`;
        status.textContent = 'مواقيت بغداد — تتحدث تلقائيًا حسب الوقت الحالي';
    } catch (error) {
        status.textContent = 'تعذر حساب مواقيت الصلاة حاليًا.';
        list.innerHTML = '';
    }
}
