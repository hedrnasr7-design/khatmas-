...
const defaultUserSettings = { pageBg:'#fcfcf0', cardBg:'#fffdf0', primary:'#1b4332', accent:'#d4af37', text:'#111111', border:'#1b4332', allBg:'#1b4332', imagesBg:'#8b4513', favBg:'#1b4332', modalBg:'#fffdf0', modalText:'#000000', baseSize:17, khatmaSize:18, modalSize:18 };
const userSettingIds = { pageBg:'userSettingPageBg', cardBg:'userSettingCardBg', primary:'userSettingPrimary', accent:'userSettingAccent', text:'userSettingText', border:'userSettingBorder', allBg:'userSettingAllBg', imagesBg:'userSettingImagesBg', favBg:'userSettingFavBg', modalBg:'userSettingModalBg', modalText:'userSettingModalText', baseSize:'userSettingBaseSize', khatmaSize:'userSettingKhatmaSize', modalSize:'userSettingModalSize' };
const userColorTargets = [
    ['pageBg','خلفية التطبيق'],['cardBg','خلفية الخانات واللوحة'],['primary','اللون الأساسي والعناوين'],['accent','لون الزخرفة والأزرار'],['text','لون الكتابة العامة'],['border','لون الإطارات'],['allBg','خانة الختمات النصية'],['imagesBg','خانة الختمات المصورة'],['favBg','خانة الختمات المفضلة'],['modalBg','خلفية نافذة الختمة'],['modalText','كتابة نافذة الختمة'],['quotesBg','خلفية الاقتباسات'],['quotesText','كتابة الاقتباسات'],['quotesBorder','إطار الاقتباسات'],['topicBg','خلفية حديث اليوم'],['topicText','كتابة حديث اليوم'],['topicBorder','إطار حديث اليوم'],['dateBg','خلفية التاريخ'],['dateText','كتابة التاريخ'],['todayBg','خلفية ختمات اليوم'],['todayText','كتابة ختمات اليوم'],['todayBorder','إطار ختمات اليوم'],['mafatihBg','خلفية مفاتيح الجنان'],['mafatihCard','بطاقة مفاتيح الجنان'],['mafatihAccent','زخرفة مفاتيح الجنان'],['mafatihText','كتابة مفاتيح الجنان'],['mafatihBorder','إطار مفاتيح الجنان'],['mafatihHeader','رأس مفاتيح الجنان'],['mafatihTodayBg','خلفية أعمال اليوم'],['mafatihTodayAccent','عنوان أعمال اليوم'],['mafatihTodayText','كتابة أعمال اليوم'],['mafatihTodayBorder','إطار أعمال اليوم']
];
const userColorDefaults = { quotesBg:'#fffdf0', quotesText:'#1b4332', quotesBorder:'#1b4332', topicBg:'#fffdf0', topicText:'#1b4332', topicBorder:'#1b4332', dateBg:'#d9f0e2', dateText:'#1b4332', todayBg:'#fffdf0', todayText:'#1b4332', todayBorder:'#1b4332', mafatihBg:'#12141c', mafatihCard:'#1c202d', mafatihAccent:'#d4af37', mafatihText:'#f1f5f9', mafatihBorder:'#2a3042', mafatihHeader:'#1c202d', mafatihTodayBg:'#fffdf0', mafatihTodayAccent:'#d4af37', mafatihTodayText:'#1b4332', mafatihTodayBorder:'#1b4332' };
function applyUserSettings(settings = {}, persist = false) {
    const s = { ...defaultUserSettings, ...userColorDefaults, ...(settings || {}) }, root = document.documentElement;
    const vars = { pageBg:'--user-page-bg', cardBg:'--user-card-bg', primary:'--user-primary', accent:'--user-accent', text:'--user-text', border:'--user-border', allBg:'--user-all-bg', imagesBg:'--user-images-bg', favBg:'--user-fav-bg', modalBg:'--user-modal-bg', modalText:'--user-modal-text', quotesBg:'--user-quotes-bg', quotesText:'--user-quotes-text', quotesBorder:'--user-quotes-border', topicBg:'--user-topic-bg', topicText:'--user-topic-text', topicBorder:'--user-topic-border', dateBg:'--user-date-bg', dateText:'--user-date-text', todayBg:'--user-today-bg', todayText:'--user-today-text', todayBorder:'--user-today-border', mafatihTodayBg:'--user-mafatih-today-bg', mafatihTodayAccent:'--user-mafatih-today-accent', mafatihTodayText:'--user-mafatih-today-text', mafatihTodayBorder:'--user-mafatih-today-border' };
    Object.entries(vars).forEach(([key, variable]) => root.style.setProperty(variable, s[key]));
    root.style.setProperty('--user-base-size', `${Math.max(12, Math.min(32, Number(s.baseSize) || 17))}px`); root.style.setProperty('--user-khatma-size', `${Math.max(12, Math.min(40, Number(s.khatmaSize) || 18))}px`); root.style.setProperty('--user-modal-size', `${Math.max(12, Math.min(40, Number(s.modalSize) || 18))}px`);
    let style = document.getElementById('userSettingsRuntimeStyles');
    if (!style) { style = document.createElement('style'); style.id = 'userSettingsRuntimeStyles'; }
    document.head.appendChild(style);
    style.textContent = `body { background-color:var(--user-page-bg) !important; color:var(--user-text) !important; font-size:var(--user-base-size) !important; } .khatma-item { background-color:var(--user-all-bg) !important; border-color:var(--user-border) !important; font-size:var(--user-khatma-size) !important; } #listContainer[data-view="images"] .khatma-item { background-color:var(--user-images-bg) !important; } #listContainer[data-view="fav"] .khatma-item { background-color:var(--user-fav-bg) !important; } #allTab { background-color:var(--user-all-bg) !important; } #imagesTab { background-color:var(--user-images-bg) !important; } #favTab { background-color:var(--user-fav-bg) !important; } .search-container, .search-box, .quotes-box, .topic-box, #todayKhatmasBox, #dailyMessageBox, #mafatihTodayBox { background-color:var(--user-card-bg) !important; } .quotes-box { color:var(--user-quotes-text) !important; border-color:var(--user-quotes-border) !important; } .topic-box { color:var(--user-topic-text) !important; border-color:var(--user-topic-border) !important; } #todayKhatmasBox { color:var(--user-today-text) !important; border-color:var(--user-today-border) !important; } #mafatihTodayBox { background-color:var(--user-mafatih-today-bg) !important; color:var(--user-mafatih-today-text) !important; border-color:var(--user-mafatih-today-border) !important; } .modal-card { background-color:var(--user-modal-bg) !important; font-size:var(--user-modal-size) !important; border-color:var(--user-border) !important; } .modal-body { color:var(--user-modal-text) !important; font-size:var(--user-modal-size) !important; } .modal-header { background-color:var(--user-primary) !important; color:var(--user-accent) !important; } .tabs-container, .search-container, .quotes-box, .topic-box, #todayKhatmasBox, #dailyMessageBox, #mafatihTodayBox { border-color:var(--user-border) !important; } .action-btn, .close-btn { border-color:var(--user-primary) !important; }`;
    Object.entries(userSettingIds).forEach(([key, id]) => { const el = document.getElementById(id); if (el) el.value = s[key]; });
    const target = document.getElementById('userSettingColorTarget'), picker = document.getElementById('userSettingColorPicker'), output = document.getElementById('userSettingColorValue');
    const selectedKey = target?.value || 'pageBg'; const selectedColor = s[selectedKey] || userColorDefaults[selectedKey] || '#1b4332';
    if (picker) picker.value = selectedColor; if (output) output.textContent = selectedColor;
    if (persist) { localStorage.setItem('khatmas_user_device_settings', JSON.stringify(s)); const status = document.getElementById('userSettingsStatus'); if (status) status.textContent = 'تم حفظ الإعدادات على هذا الجهاز فقط.'; }
}
function loadUserSettings() { let saved = {}; try { saved = JSON.parse(localStorage.getItem('khatmas_user_device_settings') || '{}') || {}; } catch (error) {} applyUserSettings(saved); }
window.openUserSettings = () => { loadUserSettings(); const panel = document.getElementById('userSettingsPanel'); if (panel) panel.style.display = 'flex'; };
window.closeUserSettings = () => { const panel = document.getElementById('userSettingsPanel'); if (panel) panel.style.display = 'none'; };
window.saveUserSettings = () => { const values = {}; try { Object.assign(values, JSON.parse(localStorage.getItem('khatmas_user_device_settings') || '{}')); } catch (error) {} Object.entries(userSettingIds).forEach(([key, id]) => { const el = document.getElementById(id); values[key] = el?.type === 'number' ? Number(el.value) : el?.value; }); applyUserSettings(values, true); };
window.resetUserSettings = () => { localStorage.removeItem('khatmas_user_device_settings'); applyUserSettings(defaultUserSettings); const status = document.getElementById('userSettingsStatus'); if (status) status.textContent = 'تم إرجاع إعدادات هذا الجهاز إلى الافتراضي.'; };
window.syncUserSettingColorPicker = () => { const target = document.getElementById('userSettingColorTarget'), picker = document.getElementById('userSettingColorPicker'), output = document.getElementById('userSettingColorValue'); let saved = {}; try { saved = JSON.parse(localStorage.getItem('khatmas_user_device_settings') || '{}') || {}; } catch (error) {} const color = saved[target?.value] || defaultUserSettings[target?.value] || userColorDefaults[target?.value] || '#1b4332'; if (picker) picker.value = color; if (output) output.textContent = color; };
window.applyUserSettingColor = color => { const target = document.getElementById('userSettingColorTarget'); if (!target) return; const values = {}; try { Object.assign(values, JSON.parse(localStorage.getItem('khatmas_user_device_settings') || '{}')); } catch (error) {} values[target.value] = color; localStorage.setItem('khatmas_user_device_settings', JSON.stringify({ ...defaultUserSettings, ...userColorDefaults, ...values })); applyUserSettings(values); const picker = document.getElementById('userSettingColorPicker'), output = document.getElementById('userSettingColorValue'); if (picker) picker.value = color; if (output) output.textContent = color; };
...
function installUserSettingsColorPalette() {
    const target = document.getElementById('userSettingColorTarget'), palette = document.getElementById('userSettingColorPalette'), picker = document.getElementById('userSettingColorPicker');
    if (!target || !palette || !picker || target.dataset.ready === '1') return;
    target.dataset.ready = '1';
    userColorTargets.forEach(([key, label]) => target.add(new Option(label, key)));
    target.addEventListener('change', () => window.syncUserSettingColorPicker());
    picker.addEventListener('change', () => window.applyUserSettingColor(picker.value));
    basicPaletteColors.forEach(([color, label]) => { const button = document.createElement('button'); button.type = 'button'; button.title = label; button.textContent = label; button.style.backgroundColor = color; if (['#ffffff','#f9a825','#fff9c4','#ffe0b2','#e1bee7','#f8bbd0','#d7ccc8'].includes(color)) button.style.color = '#111'; button.addEventListener('click', () => window.applyUserSettingColor(color)); palette.appendChild(button); });
    window.syncUserSettingColorPicker();
}
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', installUserSettingsColorPalette);
else setTimeout(installUserSettingsColorPalette, 0);
