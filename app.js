...
function renderTodayKhatmas() {
    ...
    const labels = rows.map(row => `${row.info.number ? toArabicNum(row.info.number) + ' — ' : ''}${getScheduleDisplayTitle(row.info)}${row.info.hasImage ? ' — ختمة مصورة' : ''}`).slice(0, 80);
...
window.saveDailyMessages = async () => {
    ...
    try { await db.ref('appSettings/dailyMessages').set(values); dailyMessages = values; renderDailyMessage(); if (status) status.textContent = 'تم حفظ صور رسالة اليوم.'; }
