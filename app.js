const firebaseConfig = {
        ...
        async function syncDataFromNetwork() {
            ...
            try {
                // طلب مباشر من Firebase دون استخدام ETag أو نسخة قديمة محفوظة.
                ...
                // اعرض النسخة الجديدة فور اكتمال تنزيلها، ولا تنتظر IndexedDB.
                allKhatmas = data;
                localStorage.removeItem('offline_khatmas_perfect');
                stopLoadingProgress();
                renderCurrentData();

                // احفظ النسخة الجديدة محليًا في الخلفية للاستخدام في الزيارة القادمة.
                updateProgressUI(100);
                OfflineStore.replaceAllKhatmas(data)
                    .then(() => OfflineStore.setMeta(
                        'last-successful-sync',
                        new Date().toISOString()
                    ))
                    .catch((storageError) => {
                        console.warn('تعذر تحديث التخزين المحلي:', storageError);
                    });

                console.log('تم جلب أحدث البيانات من Firebase بنجاح');
                return true;
            } catch (error) {
                const message = error.name === 'AbortError'
                    ? 'انتهت مهلة الاتصال بـ Firebase بعد 45 ثانية.'
                    : error.message;
                console.warn(
                    'تعذر الوصول إلى Firebase، ستُستخدم النسخة المحلية:',
                    message
                );
                return false;
            } finally {
        ...
        let realtimeListenerStarted = false;
        ...
        function startRealtimeKhatmasListener() {
            if (realtimeListenerStarted || !navigator.onLine) return;

            realtimeListenerStarted = true;
            const khatmasRef = db.ref('khatmas');

            // عند التشغيل الأول تصل العناصر الحالية واحدًا واحدًا،
            // وبعد ذلك تصل الإضافات والتعديلات والحذف كتغييرات صغيرة فقط.
            khatmasRef.on('child_added', (snapshot) => {
                const key = snapshot.key;
                const value = snapshot.val() || {};

                allKhatmas[key] = value;
                lastSyncAttempt = Date.now();
                scheduleRealtimeRender();

                OfflineStore.putKhatma(key, value).catch((error) => {
                    console.warn('تعذر حفظ الختمة الجديدة محليًا:', error);
                });

                console.log(`وصلت ختمة جديدة من Firebase: ${key}`);
            }, (error) => {
                realtimeListenerStarted = false;
                console.warn('تعذر الاستماع إلى الإضافات من Firebase:', error);
            });

            khatmasRef.on('child_changed', (snapshot) => {
                const key = snapshot.key;
                const value = snapshot.val() || {};
                ...
                OfflineStore.putKhatma(key, value).catch((error) => {
                    console.warn('تعذر حفظ تعديل الختمة محليًا:', error);
                });

                console.log(`وصل تعديل من Firebase: ${key}`);
            }, (error) => {
                realtimeListenerStarted = false;
                console.warn('تعذر الاستماع إلى تعديلات Firebase:', error);
            });

            khatmasRef.on('child_removed', (snapshot) => {
                const key = snapshot.key;
                ...
                OfflineStore.deleteKhatma(key).catch((error) => {
                    console.warn('تعذر حذف الختمة محليًا:', error);
                });

                console.log(`حُذفت ختمة من Firebase: ${key}`);
            }, (error) => {
                realtimeListenerStarted = false;
                console.warn('تعذر الاستماع إلى عمليات الحذف في Firebase:', error);
            });
        }
        ...
        async function initializeOfflineFirstApp() {
            ...
            // اعرض التخزين المحلي أولًا حتى لا ينتظر المستخدم تنزيل Firebase.
            const cachedSucceeded = await loadCachedData();

            if (cachedSucceeded) {
                stopLoadingProgress();

                // نفّذ التحديث في الخلفية بعد ظهور التطبيق مباشرة.
                setTimeout(() => {
                    startRealtimeKhatmasListener();
                }, 0);
                return;
            }

            // في أول تشغيل، لا توجد نسخة محلية؛ ننتظر Firebase فقط عند الضرورة.
            const networkSucceeded = await syncDataFromNetwork();
            ...
            // بعد أول تحميل، استقبل أي إضافة أو تعديل تلقائيًا.
            startRealtimeKhatmasListener();
        }

        function syncIfDue() {
            // مستمع Firebase الحي يتولى التحديث؛ استخدم REST فقط إذا لم يبدأ المستمع.
            if (!realtimeListenerStarted && Date.now() - lastSyncAttempt >= 60000) {
                syncDataFromNetwork();
            }
        }

        window.syncKhatmasFromNetwork = syncDataFromNetwork;
        window.addEventListener('online', startRealtimeKhatmasListener);
        ...
        if ('serviceWorker' in navigator) {
            navigator.serviceWorker.addEventListener('controllerchange', () => {
                if (!window.__khatmasAppReloaded) {
                    window.__khatmasAppReloaded = true;
