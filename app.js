...
const ACCESS_VERSION = 'single-full-v2';
const savedAccessCookie = document.cookie.split('; ').find(item => item.startsWith('khatmas_access_mode='));
let accessMode = '';
try { accessMode = localStorage.getItem('khatmas_access_mode') || ''; } catch (error) {}
if (!accessMode) { try { accessMode = sessionStorage.getItem('khatmas_access_mode') || ''; } catch (error) {} }
...
function applyAccessMode(mode) {
    accessMode = mode;
    if (mode === 'admin') { localStorage.removeItem('khatmas_access_mode'); try { sessionStorage.removeItem('khatmas_access_mode'); } catch (error) {} }
    else if (mode) {
        localStorage.setItem('khatmas_access_mode', mode);
        try { sessionStorage.setItem('khatmas_access_mode', mode); } catch (error) {}
        localStorage.setItem('khatmas_access_version', ACCESS_VERSION);
