...
function renderTodayKhatmas() {
    ...
    const dailyPattern = /(?:يوميا|يوميًا|كل\s+يوم|ليلة|الاحد|الأحد|الاثنين|الإثنين|الثلاثاء|الاربعاء|الأربعاء|الخميس|الجمعة|السبت|الصبح|الفجر|السحر|الزوال|الظهر|الظهرين|العصر|المغرب|الغروب|العشاء|شروق\s+الشمس|الشروق|الضحى|صباح|مساء|بعد\s+صلاة|قبل\s+صلاة)/i;
    const rows = getScheduleRows('').filter(row => {
        const source = `${row.info.title}\n${row.info.content}`;
        return dailyPattern.test(source) || row.info.weekdays.length || row.info.periods.length;
    });
