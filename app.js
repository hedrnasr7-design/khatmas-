...
const ACCESS_VERSION = 'numeric-device-bound-v2';
...
async function claimAccessCode(code) {
    const normalized = String(code || '').normalize('NFKC').replace(/\D/g, '').slice(0, 8);
    ...
    const ref = db.ref(`appSettings/numericAccessCodes/${canonical}`);
    let result;
    try {
...
async function validateAccessCodeOnline(code, version=0) {
    ...
    try { const snap=await db.ref(`appSettings/numericAccessCodes/${code}`).once('value'); const value=snap.val(); if (value && (value.enabled===false || (Number(value.resetVersion)||0)!==Number(version) || (!value.open && value.deviceId && value.deviceId!==getDeviceId()))) return false; } catch(e) {}
    return true;
}
...
window.loadAccessCodesAdmin = async () => {
    ...
    try { const snap=await db.ref('appSettings/numericAccessCodes').once('value'); const data=snap.val()||{}; box.value=[...ACCESS_CODES,...OPEN_ACCESS_CODES].map(code=>{const v=data[code]||{}; return `${code} | ${v.enabled===false?'موقوف':'فعال'} | ${v.deviceId?'مرتبط بجهاز':'غير مستخدم'}`;}).join('\n'); } catch(e) { box.value='تعذر تحميل حالة الأكواد.'; }
};
window.disableAccessCode = async () => { const code=String(document.getElementById('accessCodeToDisable')?.value||'').trim().toUpperCase(); const status=document.getElementById('accessCodesAdminStatus'); if(!ACCESS_CODES.includes(code) && !OPEN_ACCESS_CODES.includes(code)){if(status)status.textContent='اكتب كودًا صحيحًا.';return;} try { await db.ref(`appSettings/numericAccessCodes/${code}/enabled`).set(false); if(status)status.textContent='تم إيقاف الكود.'; loadAccessCodesAdmin(); } catch(e){if(status)status.textContent='تعذر الإيقاف.';} };
window.enableAccessCode = async () => { const code=String(document.getElementById('accessCodeToDisable')?.value||'').trim().toUpperCase(); const status=document.getElementById('accessCodesAdminStatus'); if(!ACCESS_CODES.includes(code) && !OPEN_ACCESS_CODES.includes(code)){if(status)status.textContent='اكتب كودًا صحيحًا.';return;} try { await db.ref(`appSettings/numericAccessCodes/${code}`).transaction(current => ({...(current||{}), enabled:true, resetVersion:(Number(current?.resetVersion)||0)+1})); if(status)status.textContent='تم تفعيل الكود؛ سيُطلب الكود من المستخدم مرة أخرى.'; loadAccessCodesAdmin(); } catch(e){if(status)status.textContent='تعذر التفعيل.';} };
