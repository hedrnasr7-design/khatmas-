...
const ACCESS_CODES = { limited: '0B115766', tabsOnly: 'D60485B8', full: '4E155583' };
let accessMode = localStorage.getItem('khatmas_access_mode') || '';
let appStarted = false;

function applyAccessMode(mode) {
    accessMode = mode;
    if (mode) localStorage.setItem('khatmas_access_mode', mode);
...
function startAppAfterAccess() {
    ...
    if (accessMode === 'full' && 'storage' in navigator && 'persist' in navigator.storage) navigator.storage.persist().catch(() => {});
    if (accessMode === 'full' && 'serviceWorker' in navigator) {
...
window.activateAccessCode = () => {
    ...
    const mode = value === ACCESS_CODES.limited ? 'limited' : value === ACCESS_CODES.tabsOnly ? 'tabsOnly' : value === ACCESS_CODES.full ? 'full' : '';
...
if (accessMode) applyAccessMode(accessMode);
...
function explainGeminiRenameError(error) {
        ...
        async function syncDataFromNetwork() {
            ...
            try {
                const previousEtag = accessMode === 'full' ? await OfflineStore.getMeta('firebase-etag') : null;
                ...
                if (accessMode === 'full') await OfflineStore.replaceAllKhatmas(data);
...
async function initializeOfflineFirstApp() {
            ...
            if (accessMode === 'full') await loadCachedData();
...
function renderScheduleResults(rows) {
    ...
    box.innerHTML = `<table class="date-schedule-table"><thead><tr><th>الرقم</th><th>العنوان</th><th>اليوم المستخرج</th><th>الوقت المستخرج</th><th>وقت مخصص</th><th>النوع</th></tr></thead><tbody>${rows.map(row => {
        const i = row.info;
...
function setAppPage(page) {
    ...
    if (!pageTwo || !controls || !tabs || !sticky) return;
    if (page === 2) {
        pageTwo.append(controls, tabs);
...
(() => {
    ...
    document.addEventListener('touchend', event => {
        ...
        if (Math.abs(dx) < 55 || Math.abs(dx) < Math.abs(dy) * 1.2) return;
        setAppPage(document.body.classList.contains('page-two-active') ? 1 : 2);
    }, { passive: true });
})();
...
function renderTodayKhatmas() {
    ...
    const extra = String(window.todayKhatmasSettings?.text || '').trim();
    const text = [extra, labels.length ? labels.join(' • ') : 'لا توجد ختمات مبرمجة لهذا اليوم'].filter(Boolean).join(' • ');
    track.innerHTML = `<span>${escapeHtml(text)}</span><span aria-hidden="true">${escapeHtml(text)}</span>`;
}

window.todayKhatmasSettings = { text: '', bg: '#fffdf0', textColor: '#1b4332', border: '#1b4332', size: 20, speed: 28 };
function applyTodayKhatmasSettings(settings) {
    ...
    const track = document.getElementById('todayKhatmasTrack');
    if (track) track.style.animationDuration = `${Math.max(5, Math.min(120, Number(s.speed) || 28))}s`;
...
window.saveTodayKhatmasSettings = async () => {
    ...
    try { await db.ref('appSettings/todayKhatmas').set({ ...settings, updatedAt: firebase.database.ServerValue.TIMESTAMP }); applyTodayKhatmasSettings(settings); document.getElementById('todayKhatmasStatus').textContent = 'تم حفظ إعدادات ختمات اليوم.'; }
    catch (error) { document.getElementById('todayKhatmasStatus').textContent = 'تعذر الحفظ؛ تأكد من دخول المشرف.'; }
};
async function loadTodayKhatmasSettings() { try { const snapshot = await db.ref('appSettings/todayKhatmas').once('value'); applyTodayKhatmasSettings(snapshot.val() || {}); } catch (error) { applyTodayKhatmasSettings({}); } }
...
function renderDailyMessage() {
    ...
    image.src = dailyMessages[dayNumber % dailyMessages.length];
    box.style.display = 'block';
}
async function loadDailyMessages() {
    try {
        const snapshot = await db.ref('appSettings/dailyMessages').once('value');
        dailyMessages = Array.isArray(snapshot.val()) ? snapshot.val().filter(Boolean) : Object.values(snapshot.val() || {}).filter(Boolean);
        const input = document.getElementById('dailyMessagesInput');
        if (input) input.value = dailyMessages.join('\n');
        renderDailyMessage();
...
window.saveDailyMessages = async () => {
    ...
    const file = document.getElementById('dailyMessageFileInput')?.files?.[0];
    try {
        if (file) values.push(await compressDailyMessageImage(file));
        await db.ref('appSettings/dailyMessages').set(values); dailyMessages = values; renderDailyMessage(); if (status) status.textContent = 'تم حفظ صور رسالة اليوم.';
    }
...
function renderDateBar() {
    ...
    if (live) {
        live.textContent = `${dateText} ${formatted.gregorian}\n${formatted.hijri} • ${formatIraqClock(iraqNow)}`;
    ...
    const movingText = [extra, scheduleText].filter(Boolean).join(' • ');
