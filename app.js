...
function setAppPage(page) {
    ...
    const pageNav = document.getElementById('appPageNav');
    if (pageNav) pageNav.querySelectorAll('[data-app-page]').forEach(button => button.classList.toggle('active', Number(button.dataset.appPage) === page));
    const controls = document.querySelector('.search-container');
    const tabs = document.querySelector('.tabs-container');
...
(() => {
    ...
    document.addEventListener('touchend', event => {
        ...
        setAppPage(dx < 0 ? (current === 6 ? 1 : current + 1) : (current === 1 ? 6 : current - 1));
    }, { passive: true });
