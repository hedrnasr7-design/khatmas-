...
let accessMode = localStorage.getItem('khatmas_access_mode') || (savedAccessCookie ? decodeURIComponent(savedAccessCookie.split('=')[1]) : '');
...
async function initializeOfflineFirstApp() {
        ...
        const topicRules = [
            ['قضاء الحوائج', /قضاء الحوائج|قضاء الحاجة|الحوائج|الحاجات|حاجة/],
            ['الرزق والمال', /رزق|الرزاق|المال|الغنى|الفقر|الدين|الديون|التجارة|البيع|الشراء/],
            ['الشفاء والصحة', /شفاء|شافي|مريض|المرض|مرض|الصحة|العافية|الوجع|الألم/],
            ['الزواج والمحبة', /زواج|الزوجه|الزوجة|الزوج|محبة|المحبة|القبول|المودة|العشق|الخطبة/],
            ['النجاح والدراسة', /نجاح|طالب|الطلاب|امتحان|الدراسة|الدراسي|التوفيق|التميز|الاختبار/],
            ['العمل والوظيفة', /عمل|وظيفة|الوظيف|المهنة|الرزق|المشروع|العملاء|المعاملات/],
            ['المشاكل والهموم', /مشكلة|مشاكل|هموم|الهم|الغم|الكرب|الضيق|الحزن|الحيرة|الفرج/],
            ['الحماية ودفع الضرر', /حسد|عين|سحر|مس|عدو|أعداء|شر|خوف|حماية|تحصين|دفع البلاء/],
            ['السفر والعودة', /سفر|مسافر|الطريق|العودة|الغائب|غائب/],
            ['الذرية والأطفال', /ذرية|حمل|الحمل|ولادة|مولود|طفل|الأطفال|النسل/],
            ['المحاكم والحقوق', /محكمة|المحاكم|قضية|قضايا|حق|حقوق|خصومة|الشرطة|السجن/],
            ['العبادة والقرآن والأدعية', /قرآن|سورة|آية|دعاء|ذكر|صلاة|زيارة|زيارات|تسبيح|استغفار|ختمة/]
        ];
        function getKhatmaTopics(record) {
            ...
            const matches = topicRules.filter(([, pattern]) => pattern.test(text)).map(([name]) => name);
            return matches.length ? matches : ['عام ومتفرقات'];
        }
        ...
        function renderTopicTabs(data, view) {
            ...
            tabs.querySelectorAll('[data-topic]').forEach(button => button.addEventListener('click', () => { activeTopicByView[view] = button.dataset.topic; filterKhatmas(); }));
        };
        window.renderList = (data, view = 'all') => {
            ...
            sortedKeys.forEach(key => {
                ...
                htmlContent += `<div class="khatma-item"><div class="khatma-title" onclick="show('${key}')">${escapeHtml(displayTitle)}${typeLabel}</div><button onclick="toggleFav('${key}')" style="background:none; border:none; font-size:20px; cursor:pointer;">${isFav}</button></div>`;
            });
        ...
        window.reclassifyAdminTopics = async () => {
            try { await requireAdminSession(); } catch (error) { alert('انتهت جلسة المشرف. أغلق اللوحة وسجّل الدخول من جديد.'); return; }
            ...
            if (!rows.length) return alert('لا توجد نتائج لإعادة تصنيفها.');
            if (!confirm(`سيتم مسح التصنيف اليدوي وإعادة ${rows.length} ختمة إلى التصنيف التلقائي. هل تريد المتابعة؟`)) return;
            try {
                const updates = {};
                rows.forEach(([key, record]) => { updates[`khatmas/${key}/customTopic`] = null; record.customTopic = ''; });
                await db.ref().update(updates);
                await Promise.all(rows.map(([key, record]) => OfflineStore.putKhatma(key, record)));
                renderAdminTopicBrowser();
                alert('تمت إعادة التصنيف التلقائي للنتائج.');
            } catch (error) { console.error(error); alert(`تعذر إعادة التصنيف: ${error.code || error.message || 'تحقق من صلاحيات قاعدة البيانات.'}`); }
        };
        window.renderAdminTopicBrowser = () => {
            ...
            browser.innerHTML = groups.size ? [...groups.entries()].sort((a,b) => a[0].localeCompare(b[0], 'ar')).map(([topic, rows]) => `<div class="admin-topic-card" style="--topic-color:${topicColor(topic)};--topic-bg:${topicBackground(topic)};"><strong>${escapeHtml(topic)} <small>(${rows.length})</small></strong>${rows.sort((a,b) => extractNumber(a[1].title)-extractNumber(b[1].title)).map(([key, record]) => { const image = record.image && String(record.image).trim(); const label = image ? (getImageDisplayTitles()[key] || record.title || 'بدون عنوان') : (record.title || 'بدون عنوان'); return `<div class="admin-topic-row"><span>${escapeHtml(label)}</span><button type="button" class="admin-topic-edit" onclick="edit('${key}')">تعديل الموضوع</button></div>`; }).join('')}</div>`).join('') : '<p style="color:#666;">لا توجد نتائج مطابقة.</p>';
        };
...
function renderTodayKhatmas() {
    ...
    track.innerHTML = `<span>${escapeHtml(text)}</span>`;
}
...
function applyTodayKhatmasSettings(settings) {
    ...
    if (track) {
        ...
        track.style.animationDuration = `${Math.round(240 - (level - 1) * (216 / 99))}s`;
...
window.updateTodayKhatmasMotionLabel = value => { const n = Math.max(1, Math.min(100, Number(value) || 50)); const label = n < 25 ? 'بطيء جدًا' : n < 45 ? 'بطيء' : n < 65 ? 'متوسط' : n < 85 ? 'سريع' : 'سريع جدًا'; const el = document.getElementById('todayKhatmasMotionLabel'); if (el) el.textContent = `${n} / 100 — ${label}`; };
window.saveTodayKhatmasSettings = async () => { try { await requireAdminSession(); } catch (error) { document.getElementById('todayKhatmasStatus').textContent = 'انتهت جلسة المشرف؛ سجّل الدخول من جديد ثم حاول.'; return; } const settings = { text: document.getElementById('todayKhatmasTextInput')?.value.trim() || '', bg: document.getElementById('todayKhatmasBgColor')?.value || '#fffdf0', textColor: document.getElementById('todayKhatmasTextColor')?.value || '#1b4332', border: document.getElementById('todayKhatmasBorderColor')?.value || '#1b4332', size: Number(document.getElementById('todayKhatmasSize')?.value) || 18, motionLevel: Number(document.getElementById('todayKhatmasMotionLevel')?.value) || 50 };
