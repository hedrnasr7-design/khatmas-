...
async function initializeOfflineFirstApp() {
        ...
        function renderTopicTabs(data, view) {
            ...
            if (!tabs) return;
            // التصنيف اليدوي/الموضوعي متاح للمشرف فقط داخل لوحة الإدارة.
            tabs.classList.remove('visible');
            tabs.innerHTML = '';
            return;
            if (view === 'fav' || view === 'general') {
            ...
            tabs.innerHTML = `<button type="button" class="topic-tab ${active === 'all' ? 'active' : ''}" data-topic="all">تسلسل الختمات العام <small>(${Object.keys(data || {}).length})</small></button>` + topics.map(([topic, count]) => `<button type="button" class="topic-tab ${active === topic ? 'active' : ''}" data-topic="${escapeHtml(topic)}">${escapeHtml(topic)} <small>(${count})</small></button>`).join('');
            tabs.classList.toggle('visible', topics.length > 0);
        ...
        window.reclassifyAdminTopics = async () => {
            ...
            } catch (error) { alert('تعذر إعادة التصنيف. تأكد من تسجيل الدخول واتصال الإنترنت.'); }
        };
...
window.saveTodayKhatmasSettings = async () => { const settings = { text: document.getElementById('todayKhatmasTextInput')?.value.trim() || '', bg: document.getElementById('todayKhatmasBgColor')?.value || '#fffdf0', textColor: document.getElementById('todayKhatmasTextColor')?.value || '#1b4332', border: document.getElementById('todayKhatmasBorderColor')?.value || '#1b4332', size: Number(document.getElementById('todayKhatmasSize')?.value) || 18, motionLevel: Number(document.getElementById('todayKhatmasMotionLevel')?.value) || 50 };
