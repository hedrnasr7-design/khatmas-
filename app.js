...
const OPEN_ACCESS_CODES = ['06096245'];
const ACCESS_CODES = [];
const ACCESS_VERSION = 'numeric-local-v4';
...
async function claimAccessCode(code) {
    ...
    if (!isOpen || !/^\d{8}$/.test(normalized)) throw new Error('الكود غير صحيح.');
    const deviceId = getDeviceId();
    const key = `khatmas_local_code_${normalized}`;
    let value = {};
    try { value = JSON.parse(localStorage.getItem(key) || '{}'); } catch (e) { value = {}; }
    if (value.enabled === false) throw new Error('هذا الكود موقوف.');
    value = { ...value, enabled: true, open: true, deviceId: null, claimedAt: value.claimedAt || Date.now() };
...
async function validateAccessCodeOnline(code, version = 0) {
    if (!code) return true;
    try {
        const value = JSON.parse(localStorage.getItem(`khatmas_local_code_${code}`) || 'null');
        return Boolean(value) && value.enabled !== false && (Number(value.resetVersion) || 0) === Number(version || 0);
...
const storedAccessVersion = localStorage.getItem('khatmas_access_version');
if (storedAccessVersion !== ACCESS_VERSION) {
    // إجبار كل نسخة جديدة على طلب الكود من جديد وعدم توريث جلسة كود قديم.
    try {
        localStorage.removeItem(ACCESS_STORAGE_KEY);
        sessionStorage.removeItem(ACCESS_STORAGE_KEY);
        localStorage.removeItem(ACCESS_CREDENTIAL_KEY);
        localStorage.removeItem('khatmas_numeric_access_mode');
        document.cookie = 'khatmas_access_mode=; Max-Age=0; path=/; SameSite=Lax';
        document.cookie = 'khatmas_numeric_access_mode=; Max-Age=0; path=/; SameSite=Lax';
    } catch (error) {}
    activeAccessCode = '';
    activeAccessVersion = 0;
    accessValidationPending = false;
    accessMode = '';
...
window.loadAccessCodesAdmin = async () => {
    const box = document.getElementById('accessCodesAdminList');
    if (!box) return;
    try {
        box.value = OPEN_ACCESS_CODES.map(code => {
            let value = {};
            try { value = JSON.parse(localStorage.getItem(`khatmas_local_code_${code}`) || '{}'); } catch (e) {}
            return `${code} | ${value.enabled === false ? 'موقوف' : 'فعال'}`;
        }).join('\n');
    } catch (e) { box.value = 'تعذر تحميل حالة الكود.'; }
};
window.disableAccessCode = async () => {
    const code = normalizeAccessDigits(document.getElementById('accessCodeToDisable')?.value || '').replace(/\D/g, '').slice(0, 8);
    const status = document.getElementById('accessCodesAdminStatus');
    if (!OPEN_ACCESS_CODES.includes(code)) { if (status) status.textContent = 'اكتب الكود الصحيح.'; return; }
    const key = `khatmas_local_code_${code}`;
    let value = {};
    try { value = JSON.parse(localStorage.getItem(key) || '{}'); } catch (e) {}
    localStorage.setItem(key, JSON.stringify({ ...value, enabled: false }));
    if (status) status.textContent = 'تم إيقاف الكود على هذا الجهاز.';
    loadAccessCodesAdmin();
};
window.enableAccessCode = async () => {
    const code = normalizeAccessDigits(document.getElementById('accessCodeToDisable')?.value || '').replace(/\D/g, '').slice(0, 8);
    const status = document.getElementById('accessCodesAdminStatus');
    if (!OPEN_ACCESS_CODES.includes(code)) { if (status) status.textContent = 'اكتب الكود الصحيح.'; return; }
    const key = `khatmas_local_code_${code}`;
    let value = {};
    try { value = JSON.parse(localStorage.getItem(key) || '{}'); } catch (e) {}
    localStorage.setItem(key, JSON.stringify({ ...value, enabled: true, resetVersion: (Number(value.resetVersion) || 0) + 1 }));
    if (status) status.textContent = 'تم تفعيل الكود على هذا الجهاز.';
    loadAccessCodesAdmin();
};
...
const shiaOccasionsByMonthDay = {
    ...
    '4-9': ['ذكرى وفاة السيدة فاطمة المعصومة (عليها السلام) — بحسب بعض التقاويم'],
    '4-10': ['ذكرى ولادة الإمام الحسن العسكري (عليه السلام) — بحسب رواية', 'ذكرى وفاة السيدة فاطمة المعصومة (عليها السلام) — بحسب رواية'],
    '4-11': ['ذكرى ولادة الإمام الحسن العسكري (عليه السلام) — بحسب بعض التقاويم', 'ذكرى وفاة السيدة فاطمة المعصومة (عليها السلام) — بحسب بعض التقاويم'],
    '4-12': ['ذكرى وفاة السيدة فاطمة المعصومة (عليها السلام) — بحسب بعض التقاويم'],
