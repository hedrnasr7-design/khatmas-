...
async function syncDataFromNetwork() {
            ...
            try {
                ...
                allKhatmas = removeDeletedKhatmaFromData(merged);
                stopLoadingProgress();
                renderCurrentData();
                try {
...
function getHijriTodayParts() {
    ...
    } catch (error) { return { day:0, month:0, year:0 }; }
}
function getMafatihDailyItems() {
    const source = Array.isArray(window.mafatihDailyItems) ? window.mafatihDailyItems : [];
    if (!source.length) return [];
    const h = getHijriTodayParts();
    const weekday = new Date().getDay();
    const categoryByWeekday = ['duas','taqibat','ziyarat','ziyarat','duas','months','taqibat'];
    const category = categoryByWeekday[weekday];
    const monthNames = { 7:'رجب', 8:'شعبان', 9:'رمضان', 10:'شوال', 11:'ذو القعدة', 12:'ذو الحجة' };
    const monthWord = monthNames[h.month] || '';
    const monthMatches = source.filter(item => item.category === 'months' && (monthWord && String(item.name).includes(monthWord)));
    const categoryMatches = source.filter(item => item.category === category);
    const pool = [...monthMatches, ...categoryMatches, ...source.filter(item => item.category === 'months')];
    const unique = [], seen = new Set();
    pool.forEach(item => { const key = `${item.name}|${item.text}`; if (!seen.has(key)) { seen.add(key); unique.push(item); } });
    const count = Math.max(1, Math.min(8, Number(mafatihTodaySettings.count) || 3));
    const offset = h.day ? (h.day - 1) % Math.max(1, unique.length) : 0;
    return Array.from({ length: Math.min(count, unique.length) }, (_, index) => unique[(offset + index) % unique.length]);
}
function getMafatihDailyText() {
    return getMafatihDailyItems().map(item => `${item.name}\n${item.text}`).join('\n\n');
}
function applyMafatihTodaySettings(settings = {}, persist = false) {
    mafatihTodaySettings = { enabled: true, count: 3, bg: '#fffdf0', accent: '#d4af37', text: '#1b4332', border: '#1b4332', font: 'Amiri, serif', size: 18, customItems: '', displayOverride: '', ...(settings || {}) };
    ...
    const count = document.getElementById('mafatihTodayCount'); if (count) count.value = mafatihTodaySettings.count || 3;
    const custom = document.getElementById('mafatihTodayCustomItems'); if (custom) custom.value = mafatihTodaySettings.customItems || '';
    const display = document.getElementById('mafatihTodayDisplayText'); if (display) display.value = mafatihTodaySettings.displayOverride || getMafatihDailyText();
...
function renderMafatihToday() {
    ...
    const manual = s.manualText === true;
    const override = String(s.displayOverride || '').trim();
    const custom = String(s.customItems || '').trim();
    const text = manual ? String(s.displayOverride || '') : (override || custom || getMafatihDailyText());
    box.innerHTML = text ? text.split(/\n+/).filter(Boolean).map(line => `<div>${escapeHtml(line)}</div>`).join('') : '<div style="text-align:center;padding:18px;">لا توجد أعمال اليوم في مفاتيح الجنان.</div>';
}
...
window.addEventListener('mafatih-data-ready', () => { if (mafatihTodaySettings) { applyMafatihTodaySettings(mafatihTodaySettings); renderMafatihToday(); } });
...
window.saveMafatihTodaySettings = async () => {
    ...
    const enteredText = document.getElementById('mafatihTodayDisplayText')?.value || '';
    const enteredCustom = document.getElementById('mafatihTodayCustomItems')?.value || '';
    const manualText = enteredText.trim() !== getMafatihDailyText().trim() || enteredCustom.trim() !== '';
    const s = { ...applyMafatihTodaySettings({enabled:document.getElementById('mafatihTodayEnabled')?.checked, count:document.getElementById('mafatihTodayCount')?.value, bg:document.getElementById('mafatihTodayBg')?.value, accent:document.getElementById('mafatihTodayAccent')?.value, text:document.getElementById('mafatihTodayText')?.value, border:document.getElementById('mafatihTodayBorder')?.value, font:document.getElementById('mafatihTodayFont')?.value, size:document.getElementById('mafatihTodaySize')?.value, customItems:enteredCustom, displayOverride:enteredText, manualText}, true), updatedAt:Date.now() };
...
(() => {
    ...
    const ignored = event => {
        ...
        return target && target.closest && target.closest('input, textarea, select, button, a, #pageFive');
    };
    document.addEventListener('touchstart', event => {
        if (ignored(event)) { tracking = false; return; }
...
// مستمع مستقل لسحب صفحة مفاتيح الجنان؛ لا يعتمد على مستمع السحب العام.
(() => {
    let startX = 0, startY = 0, active = false;
    const page = () => document.getElementById('pageFive');
    const ignored = target => target && target.closest && target.closest('input, textarea, select, button, a');
    const begin = event => {
        const p = page(), point = event.touches?.[0] || event.changedTouches?.[0] || event;
        if (!p || document.body.dataset.appPage !== '5' || !point || ignored(event.target)) { active = false; return; }
        startX = point.clientX; startY = point.clientY; active = true;
    };
    const end = event => {
        if (!active) return;
        active = false;
