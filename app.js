...
function applyThemeSettings(font, presetName, persist = false) {
    ...
    style.textContent = `
        ...
        .khatma-item, #allTab { background-color: var(--theme-primary) !important; }
        #imagesTab { background-color: var(--theme-secondary) !important; }
        #favTab { background-color: var(--theme-primary) !important; }
    ...
    if (persist) {
        localStorage.setItem('app_theme_settings', JSON.stringify({ font: font || 'Amiri, serif', preset: presetName || 'green' }));
...
function loadThemeSettings() {
    ...
    applyThemeSettings(saved.font, saved.preset, false);
}
...
window.previewThemeSettings = () => {
    ...
    applyThemeSettings(font, preset, false);
};

window.saveThemeSettings = () => {
    const font = document.getElementById('themeFontSelect')?.value || 'Amiri, serif';
    const preset = document.getElementById('themePresetSelect')?.value || 'green';
    applyThemeSettings(font, preset, true);
};

window.resetThemeSettings = () => {
    localStorage.removeItem('app_theme_settings');
    applyThemeSettings('Amiri, serif', 'green', false);
    const status = document.getElementById('themeStatus');
...
function applyDateAppearance(settings) {
    const root = document.documentElement;
    root.style.setProperty('--date-bg', settings.bgColor || '#fffdf0');
...
function renderDateBar() {
    ...
    if (!bar) return;
    let settings = { mode: 'auto', customDate: '', bgColor: '#fffdf0', textColor: '#1b4332', font: 'Amiri, serif' };
    try {
    ...
    bar.innerHTML = `<div>${formatted.weekday}</div><div>${formatted.gregorian}</div><div>${formatted.hijri}</div>`;
}
...
function loadDateSettings() {
    ...
    if (bgColor) bgColor.value = settings.bgColor || '#fffdf0';
...
window.saveDateSettings = () => {
    ...
    const bgColor = document.getElementById('dateBgColor')?.value || '#fffdf0';
    ...
    localStorage.setItem('date_display_settings', JSON.stringify({ mode, customDate, font, bgColor, textColor }));
    renderDateBar();
    const status = document.getElementById('dateSettingsStatus');
    if (status) status.innerText = mode === 'auto' ? 'تم تثبيت التحديث التلقائي.' : 'تم حفظ التاريخ المخصص.';
};
