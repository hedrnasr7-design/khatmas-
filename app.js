...
function readThemeColorInputs() {
    ...
    ['quotesBgColor','quotesTextColor','quotesBorderColor','topicBgColor','topicTextColor','topicBorderColor','dateBgColor','dateTextColor','todayKhatmasBgColor','todayKhatmasTextColor','todayKhatmasBorderColor','mafatihBgColor','mafatihCardColor','mafatihAccentColor','mafatihTextColor','mafatihBorderColor','mafatihHeaderColor'].forEach(id => { const input = document.getElementById(id); if (input) colors[id] = input.value; });
    return colors;
}
...
function renderTodayKhatmas() {
    ...
    const value = overrideText ? overrideText.split(/\n+/).map(v => ({ label: v.trim(), key: findTodayKhatmaKeyByLabel(v), title: v.trim() })).filter(v => v.label) : generatedValue;
