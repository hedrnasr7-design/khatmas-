...
const ACCESS_VERSION = 'single-full-v3';
...
let accessMode = '';
try { accessMode = localStorage.getItem(ACCESS_STORAGE_KEY) || ''; } catch (error) {}
if (!accessMode) { try { accessMode = sessionStorage.getItem(ACCESS_STORAGE_KEY) || ''; } catch (error) {} }
...
window.activateAccessCode = () => {
    ...
    try { localStorage.setItem(ACCESS_STORAGE_KEY, mode); sessionStorage.setItem(ACCESS_STORAGE_KEY, mode); } catch (error) {}
    document.cookie = `khatmas_access_mode=${encodeURIComponent(mode)}; max-age=31536000; path=/; SameSite=Lax`;
    const gate = document.getElementById('accessGate');
    if (gate) { gate.style.display = 'none'; gate.style.pointerEvents = 'none'; }
    if (status) status.textContent = '';
    // نبدأ فتح الواجهة في دورة مستقلة حتى لا يبقى زر الدخول ممسكًا بمسار التشغيل.
    requestAnimationFrame(() => {
        try { startAppAfterAccess(); } catch (error) { console.error('تعذر فتح التطبيق:', error); const target = document.getElementById('accessGateStatus'); if (target) target.textContent = 'تعذر فتح التطبيق؛ حدّث الصفحة وحاول مرة أخرى.'; }
    });
};
