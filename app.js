...
function setAppPage(page) {
    const pages = [null, null, document.getElementById('pageTwo'), document.getElementById('pageThree'), document.getElementById('pageFour'), document.getElementById('pageFive'), document.getElementById('pageSix'), document.getElementById('pageSeven')];
    page = Math.max(1, Math.min(7, Number(page) || 1));
    ...
    if (page === 7 && pages[7]) {
        ...
        document.body.classList.add('app-page-7');
        document.body.appendChild(pages[7]);
        renderQuranBenefitsGallery();
        document.body.dataset.appPage = '7';
        return;
    }
    if (page === 6 && pageSix) {
        pageSix.hidden = false;
...
window.saveQuranBenefitsSettings = async () => {
    ...
    try {
        if (status) status.textContent = 'جارٍ ضغط الصور وحفظها لجميع المستخدمين...';
        const settings = { ...defaultQuranBenefitsSettings, ...(window.quranBenefitsSettings || {}), images: await Promise.all(files.map(compressDailyMessageImage)), updatedAt: Date.now() };
        ...
        if (input) input.value = '';
        renderQuranBenefitsAdminPreview();
        if (status) status.textContent = `تم حفظ ${toArabicNum(String(settings.images.length))} صورة لجميع المستخدمين.`;
    } catch (error) {
