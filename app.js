...
(() => {
    // مستمع واحد للسحب في كل الصفحة، بما فيها الموضوعات وصفحة مفاتيح الجنان.
    // نستثني حقول الإدخال فقط حتى لا نكسر الكتابة أو التمرير داخلها.
    ...
    const begin = event => {
        ...
        if (!point) return;
        startX = point.clientX; startY = point.clientY; tracking = true;
    ...
    const cancel = () => { tracking = false; pointerId = null; };
    ...
    // احتياط للأجهزة القديمة التي لا تدعم Pointer Events.
