...
async function renderMafatihToday() {
    ...
    try {
        ...
        target.innerHTML = selected.map((item, index) => `<div class="mafatih-today-item"><a href="#" onclick="openMafatihTodayReader(${index});return false;">${escapeHtml(item.title)}</a><div>${escapeHtml(item.text.slice(0, 180))}${item.text.length > 180 ? '…' : ''}</div></div>`).join('');
