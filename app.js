    ...
    if (user?.email?.toLowerCase() === ADMIN_EMAIL.toLowerCase()) {
        ...
        async function applyBackgroundAudio(settings, cachedRecord = null) {
            ...
            audio.onended = () => {
                ...
                applyBackgroundAudio({ url }, track).catch(() => {});
            };
        ...
        window.uploadAudioToGitHub = async () => {
            ...
            try {
                ...
                if (oldPlaylistResponse.ok) playlistSha = (await oldPlaylistResponse.json()).sha;
                const mergedTracks = [...backgroundAudioPlaylist.filter(track => track.path !== GITHUB_AUDIO_PATH), ...uploadedTracks];
                const playlistBase64 = btoa(unescape(encodeURIComponent(JSON.stringify({ version: 1, tracks: mergedTracks }, null, 2))));
                ...
                backgroundAudioPlaylist = mergedTracks;
                ...
                if (status) status.innerText = `تم رفع ${uploadedTracks.length} مقطعًا وحفظ المفتاح وقائمة التشغيل.`;
            } catch (error) {
...
function setAppPage(page) {
    const pages = [null, null, document.getElementById('pageTwo'), document.getElementById('pageThree'), document.getElementById('pageFour'), document.getElementById('pageFive'), document.getElementById('pageSix')];
    page = Math.max(1, Math.min(6, Number(page) || 1));
    ...
    const sticky = document.querySelector('.sticky-top-container');
    ['todayKhatmasBox', 'dailyMessageBox', 'mafatihTodayBox', 'mafatihTodayReader'].forEach(id => {
    ...
    document.body.classList.remove('app-page-1', 'app-page-2', 'app-page-3', 'app-page-4', 'app-page-5', 'app-page-6');
...
function initDailyBulkUploadPanel() {
    const root = document.getElementById('dailyBulkUploadRows');
    if (!root || root.dataset.ready === '1') return;
    const section = (title, rows) => `<div style="margin:4px 0 10px;"><strong style="display:block;background:#1b4332;color:#fff;padding:6px;">${title}</strong>${rows.map(row => `<label style="display:grid;grid-template-columns:125px 1fr;gap:6px;align-items:center;border-bottom:1px solid #e5e7eb;padding:6px 2px;font-size:13px;"><span>${row.label}</span><input type="file" accept="image/*" ${row.multiple ? 'multiple' : ''} data-bulk-kind="${row.kind}" data-bulk-key="${row.key}" style="width:100%;font-size:12px;"></label>`).join('')}</div>`;
    const arabic = Array.from({length:30}, (_, i) => ({kind:'arabic', key:String(i+1), label:`اليوم الهجري ${toArabicNum(String(i+1))}`, multiple:true}));
    const weekdays = bulkWeekdayNames.map((label, i) => ({kind:'prayer', key:String(i), label:`${label} — صلاة اليوم`, multiple:false}));
    const dhikr = bulkWeekdayNames.map((label, i) => ({kind:'dhikr', key:String(i), label:`${label} — أذكار اليوم`, multiple:false}));
    const dua = bulkWeekdayNames.map((label, i) => ({kind:'dua', key:String(i), label:`${label} — دعاء اليوم`, multiple:true}));
    const visit = bulkWeekdayNames.map((label, i) => ({kind:'visit', key:String(i), label:`${label} — زيارة اليوم`, multiple:true}));
    const nights = bulkWeekdayNames.map((label, i) => ({kind:'night', key:String(i), label:`ليلة ${label}`, multiple:true}));
    root.innerHTML = [section('أيام الشهر العربي (1–30)', arabic), section('صلاة اليوم', weekdays), section('أذكار اليوم', dhikr), section('دعاء اليوم', dua), section('زيارة اليوم', visit), section('صلاة الليلة', nights)].join('');
    root.dataset.ready = '1';
}

async function saveAllDailyImagesSettings() {
    const status = document.getElementById('dailyBulkUploadStatus');
    try { await requireAdminSession(); } catch (error) { if (status) status.textContent = 'انتهت جلسة المشرف؛ سجّل الدخول من جديد ثم حاول.'; return; }
    const inputs = [...document.querySelectorAll('#dailyBulkUploadRows input[type="file"]')];
    const selected = inputs.filter(input => input.files && input.files.length);
    if (!selected.length) { if (status) status.textContent = 'اختر صورة واحدة على الأقل قبل الحفظ.'; return; }
    const progress = message => { if (status) status.textContent = message; };
    try {
        progress(`جارٍ تجهيز ${selected.length} خانة وحفظها لجميع المستخدمين...`);
        const dailyCards = { ...(window.dailyCardsSettings || defaultDailyCardsSettings), days: { ...((window.dailyCardsSettings || {}).days || {}) }, titleColor: document.getElementById('bulkDailyCardsTitleColor')?.value || '#1b4332', updatedAt: Date.now() };
        const duaVisit = { ...(window.dailyDuaVisitSettings || defaultDailyDuaVisitSettings), days: { ...((window.dailyDuaVisitSettings || {}).days || {}) }, titleColor: document.getElementById('bulkDuaVisitTitleColor')?.value || '#8a4b08', updatedAt: Date.now() };
        const nightPrayer = { ...(window.nightPrayerSettings || defaultNightPrayerSettings), nights: { ...((window.nightPrayerSettings || {}).nights || {}) }, titleColor: document.getElementById('bulkNightPrayerTitleColor')?.value || '#56308f', updatedAt: Date.now() };
        const arabicMonth = { ...(window.arabicMonthSettings || defaultArabicMonthSettings), days: { ...((window.arabicMonthSettings || {}).days || {}) }, titleColor: document.getElementById('bulkArabicMonthTitleColor')?.value || '#8a4b08', updatedAt: Date.now() };
        let processed = 0;
        for (const input of selected) {
            const kind = input.dataset.bulkKind, key = input.dataset.bulkKey;
            const images = await Promise.all([...input.files].map(compressDailyMessageImage));
            if (kind === 'arabic') arabicMonth.days[key] = { images };
            else if (kind === 'prayer' || kind === 'dhikr') dailyCards.days[key] = { ...(dailyCards.days[key] || {}), [kind]: images[0] };
            else if (kind === 'dua' || kind === 'visit') duaVisit.days[key] = { ...(duaVisit.days[key] || {}), [kind]: images };
            else if (kind === 'night') nightPrayer.nights[key] = { images };
            processed += 1; progress(`تم تجهيز ${processed} من ${selected.length}...`);
        }
        progress('جارٍ الحفظ النهائي لجميع الأقسام...');
        const root = db.ref();
        await root.update({ 'appSettings/dailyCards': dailyCards, 'appSettings/dailyDuaVisit': duaVisit, 'appSettings/nightPrayer': nightPrayer, 'appSettings/arabicMonth': arabicMonth });
        const checks = await Promise.all(['dailyCards','dailyDuaVisit','nightPrayer','arabicMonth'].map(key => db.ref(`appSettings/${key}`).once('value')));
        if (checks.some(snapshot => !snapshot.exists())) throw new Error('لم تؤكد قاعدة البيانات حفظ أحد الأقسام.');
        localStorage.setItem('daily_cards_settings', JSON.stringify(dailyCards)); localStorage.setItem('daily_dua_visit_settings', JSON.stringify(duaVisit)); localStorage.setItem('night_prayer_settings', JSON.stringify(nightPrayer)); localStorage.setItem('arabic_month_settings', JSON.stringify(arabicMonth));
        window.dailyCardsSettings = dailyCards; window.dailyDuaVisitSettings = duaVisit; window.nightPrayerSettings = nightPrayer; window.arabicMonthSettings = arabicMonth;
        applyDailyCardsSettings(dailyCards); applyDailyDuaVisitSettings(duaVisit); applyNightPrayerSettings(nightPrayer); applyArabicMonthSettings(arabicMonth);
        progress(`تم رفع وحفظ ${selected.length} خانة لجميع المستخدمين بنجاح.`);
        selected.forEach(input => { input.value = ''; });
    } catch (error) {
        console.error(error);
        if (status) status.textContent = `فشل الحفظ الجماعي: ${error?.code || error?.message || 'تحقق من صلاحيات Firebase.'}`;
    }
}

const defaultDailyCardsSettings = { days: {}, titleColor: '#1b4332', titleSize: 22, updatedAt: 0 };
function currentDailyCardsDay() { return String(new Date().getDay()); }
...
async function loadDailyCardsSettings() {
    ...
    const saved = Number(local.updatedAt || 0) >= Number(remote.updatedAt || 0) ? { ...remote, ...local } : { ...local, ...remote };
    localStorage.setItem('daily_cards_settings', JSON.stringify(saved)); applyDailyCardsSettings(saved);
}
...
window.saveDailyCardsSettings = async () => {
    ...
    try {
        ...
        localStorage.setItem('daily_cards_settings', JSON.stringify(settings)); window.dailyCardsSettings = settings; applyDailyCardsSettings(settings);
...
async function loadDailyDuaVisitSettings() {
    ...
    const saved = Number(local.updatedAt || 0) >= Number(remote.updatedAt || 0) ? { ...remote, ...local } : { ...local, ...remote };
    localStorage.setItem('daily_dua_visit_settings', JSON.stringify(saved)); applyDailyDuaVisitSettings(saved);
}
window.saveDailyDuaVisitSettings = async () => {
    ...
    try {
        ...
        localStorage.setItem('daily_dua_visit_settings', JSON.stringify(settings)); window.dailyDuaVisitSettings = settings; applyDailyDuaVisitSettings(settings);
...
function applyNightPrayerSettings(settings) {
    ...
    const night = merged.nights[currentDailyCardsDay()] || {};
    ...
    const dayInput = document.getElementById('nightPrayerDay'); if (dayInput) dayInput.value = currentDailyCardsDay();
...
async function loadNightPrayerSettings() {
    ...
    const saved = Number(local.updatedAt || 0) >= Number(remote.updatedAt || 0) ? { ...remote, ...local } : { ...local, ...remote };
    localStorage.setItem('night_prayer_settings', JSON.stringify(saved)); applyNightPrayerSettings(saved);
}
window.saveNightPrayerSettings = async () => {
    ...
    try {
        ...
        localStorage.setItem('night_prayer_settings', JSON.stringify(settings)); window.nightPrayerSettings = settings; applyNightPrayerSettings(settings);
...
async function loadArabicMonthSettings() {
    ...
    const saved = Number(local.updatedAt || 0) >= Number(remote.updatedAt || 0) ? { ...remote, ...local } : { ...local, ...remote };
    localStorage.setItem('arabic_month_settings', JSON.stringify(saved)); applyArabicMonthSettings(saved);
}
window.saveArabicMonthDaySettings = async () => {
    ...
    try {
        ...
        localStorage.setItem('arabic_month_settings', JSON.stringify(settings)); window.arabicMonthSettings = settings; applyArabicMonthSettings(settings);
