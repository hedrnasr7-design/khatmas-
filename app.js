...
async function renderMafatihToday() {
    ...
    try {
        ...
        if (visibleText) {
            ...
            target.innerHTML = selected.map((item,index) => `<div class="mafatih-today-item"><div>${escapeHtml(item.title)}</div><a href="#" onclick="openMafatihTodayReader(${index});return false;">المزيد</a></div>`).join('');
