...
function startAppAfterAccess() {
    ...
    appStarted = true;
    // تهيئة الصفحة الرئيسية بعد فتح التطبيق مباشرة؛ لا نعتمد على حدث load وحده.
    setTimeout(() => {
        if (typeof setAppPage === 'function') {
            try { setAppPage(1); } catch (error) { console.error('تعذر تهيئة الصفحة الرئيسية:', error); }
        }
    }, 0);
    ...
    // تعطيل Service Worker القديم/غير الموجود حتى يعمل التطبيق مباشرة من GitHub Pages.
...
async function syncDataFromNetwork() {
            // تُرفع بيانات الختمات على أجزاء صغيرة لتوافق حد GitHub، ثم تُدمج هنا.
            ...
            try {
                ...
                for (let part = 1; part <= 6; part++) {
                    ...
                    try {
                        const response = await fetch(`./khatmas-index-${part}.json?v=113`, { cache: 'no-store', signal: controller.signal });
                        if (!response.ok) throw new Error(`الجزء ${part}: HTTP ${response.status}`);
                        Object.assign(merged, (await response.json()) || {});
                        loadedParts++;
                    } finally { clearTimeout(timeout); }
                }
                if (loadedParts !== 6 || Object.keys(merged).length === 0) throw new Error('ملفات الختمات المجزأة غير مكتملة');
...
function setAppPage(page) {
    ...
    document.body.classList.remove('app-page-2', 'app-page-3', 'app-page-4', 'app-page-5', 'home-list-hidden');
    ...
    // قائمة الختمات تخص صفحات التصفح فقط، ولا تظهر في الصفحة الرئيسية.
    if (page >= 2 && page <= 4 && pages[page]) {
        pages[page].append(controls, tabs, topicTabs, list);
        list.style.display = 'block';
        document.body.classList.remove('home-list-hidden');
        document.body.classList.add(`app-page-${page}`);
        if (page === 5) {
            document.body.appendChild(list);
            list.style.display = 'none';
            document.body.classList.add('home-list-hidden');
            if (typeof window.mafatihInitPage === 'function') window.mafatihInitPage();
        }
        if (page < 5) switchTab(page === 2 ? 'all' : page === 3 ? 'images' : 'fav');
        if (page < 5) { try { renderTodayKhatmas(); } catch (error) { console.warn('تعذر تحديث ختمات اليوم أثناء الانتقال، وتستمر صفحة الختمات:', error); } }
    } else {
        arrangeHomeSections();
        sticky.append(controls, tabs, topicTabs);
        // إبقاء القائمة في DOM مع إخفائها حتى لا تزاحم خانات الصفحة الرئيسية.
        document.body.appendChild(list);
        list.style.display = 'none';
        document.body.classList.add('home-list-hidden');
        switchTab('all');
...
(() => {
    ...
    const begin = event => {
        ...
        if (target && target.closest && target.closest('input, textarea, select, button, a, #pageFive')) { active = false; return; }
...
window.addEventListener('load', () => {
    if (!document.body.dataset.appPage && typeof setAppPage === 'function') {
        try { setAppPage(1); } catch (error) { console.error('تعذر تهيئة الصفحة الرئيسية:', error); }
    }
});
