...
function applyMafatihTodaySettings(settings = {}, persist = false) {
    ...
    for (const [id, value] of Object.entries({mafatihTodayEnabled:s.enabled !== false, mafatihTodayCount:s.count, mafatihTodayBg:s.bg, mafatihTodayAccent:s.accent, mafatihTodayText:s.text, mafatihTodayBorder:s.border, mafatihTodayFont:s.font, mafatihTodaySize:s.size, mafatihTodayCustomItems:s.customItems || ''})) { const el = document.getElementById(id); if (el) { if (el.type === 'checkbox') el.checked = Boolean(value); else el.value = value; } }
...
async function renderMafatihToday() {
    ...
    try {
        const customText = String(mafatihTodaySettings.customItems || '').trim();
        if (customText) {
            const selected = customText.split(/\n+/).map(line => { const parts = line.split('|').map(v => v.trim()); return { title: parts[0] || 'عمل اليوم', text: parts.slice(1).join(' | ') || '' }; }).filter(item => item.title || item.text);
            window.__mafatihTodayItems = selected;
            target.innerHTML = selected.map((item,index) => `<div class="mafatih-today-item"><strong>${escapeHtml(item.title)}</strong><div>${escapeHtml(item.text)}</div><a href="#" onclick="openMafatihTodayReader(${index});return false;">المزيد</a></div>`).join('');
            return;
        }
...
function renderTodayKhatmas() {
    ...
    const value = [...labels, ...manual].length ? [...labels, ...manual] : ['لا توجد ختمات حاليا'];
