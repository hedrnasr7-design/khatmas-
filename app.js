...
function explainGeminiRenameError(error) {
        ...
        window.switchTab = (tab, direction = null) => {
            ...
            const slideDirection = direction || (nextIndex > currentIndex ? 'left' : 'right');
            const tabsContainer = document.querySelector('.tabs-container');
            const listContainer = document.getElementById('listContainer');
            [tabsContainer, listContainer].forEach(element => {
                if (!element) return;
                element.classList.remove('screen-slide-left', 'screen-slide-right');
                void element.offsetWidth;
                element.classList.add(`screen-slide-${slideDirection}`);
            });
        ...
        (() => {
            ...
            list.addEventListener('touchend', event => {
                ...
                const next = dx < 0 ? Math.min(tabs.length - 1, current + 1) : Math.max(0, current - 1);
                if (next !== current) switchTab(tabs[next], dx < 0 ? 'left' : 'right');
            }, { passive: true });
        })();
