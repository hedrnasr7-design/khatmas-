...
function renderMafatihSections(list = mafatihSections) {
...
window.openMafatihPage = () => { setAppPage(5); renderMafatihSections(); };
window.filterMafatihSections = () => {
    const q = String(document.getElementById('mafatihSearch')?.value || '').trim().toLowerCase();
    renderMafatihSections(mafatihSections.filter(item => !q || item.title.toLowerCase().includes(q) || item.text.toLowerCase().includes(q)));
};
window.openMafatihReader = id => {
    const item = mafatihSections.find(section => section.id === id); const reader = document.getElementById('mafatihReader'); if (!item || !reader) return;
...
window.showMafatihSections = () => { const reader = document.getElementById('mafatihReader'); if (reader) reader.style.display = 'none'; renderMafatihSections(); };
window.showMafatihFavorites = () => { const ids = JSON.parse(localStorage.getItem('mafatih_favorites') || '[]'); renderMafatihSections(mafatihSections.filter(item => ids.includes(item.id))); };
