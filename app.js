...
function renderTodayKhatmas() {
    ...
    const rows = getScheduleRows('').filter(row => {
        const info = row.info;
        const hasSchedule = info.weekdays.length || info.times.length || info.periods.length || getScheduleOverride(row.key);
        if (!hasSchedule) return false;
        if (!info.weekdays.length) return true;
        return info.weekdays.includes(now.weekday) || (isIraqNight(now) && info.weekdays.includes(nextDay));
    ...
    const labels = rows.map(row => {
        ...
        const override = getScheduleOverride(row.key);
        const time = override
            ? formatArabic12Time(override.hour, override.minute)
            : (info.times[0]
                ? formatArabic12Time(info.times[0].hour, info.times[0].minute)
                : (info.periods[0]?.label || info.timePhrases[0] || 'وقت مرن'));
        return `${info.number ? toArabicNum(info.number) + ' — ' : ''}${getScheduleDisplayTitle(info)}${info.hasImage ? ' — ختمة مصورة' : ''} (${time})`;
    }).slice(0, 80);
    const value = labels.length ? labels.join(' • ') : 'لا توجد ختمات مبرمجة لهذا اليوم';
    track.innerHTML = `<span>${escapeHtml(value)} • </span><span aria-hidden="true">${escapeHtml(value)} • </span>`;
    track.style.width = 'max-content';
    const level = Math.max(1, Math.min(100, Number(window.todayKhatmasSettings?.motionLevel) || 50));
    // مدى الحركة: ١ = بطيء جدًا (٥ دقائق للدورة)، ١٠٠ = سريع نسبيًا (دقيقة).
    track.style.animationDuration = `${Math.round(300 - (level - 1) * (240 / 99))}s`;
}
