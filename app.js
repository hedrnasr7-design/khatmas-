...
function setAppPage(page) {
    const pages = [null, null, document.getElementById('pageTwo'), document.getElementById('pageThree'), document.getElementById('pageFour'), document.getElementById('pageFive'), document.getElementById('pageSix'), document.getElementById('pageSeven'), document.getElementById('pageEight')];
    page = Math.max(1, Math.min(8, Number(page) || 1));
    ...
    if ((page === 7 || page === 8) && pages[page]) {
        ...
        document.body.classList.remove('app-page-1', 'app-page-2', 'app-page-3', 'app-page-4', 'app-page-5', 'app-page-6', 'app-page-7', 'app-page-8');
...
async function uploadImagesToFirebaseStorage(files, folder, status) {
    if (!files.length) return [];
    const uploaded = [];
    for (let i = 0; i < files.length; i++) {
        if (status) status.textContent = `جارٍ ضغط الصورة ${i + 1} من ${files.length}...`;
        const file = files[i];
        // تُحفظ الصور المضغوطة كـ data URL داخل Realtime Database؛ لا حاجة إلى Storage أو خطة مدفوعة.
        const compressedDataUrl = await compressDatabaseImage(file);
        uploaded.push(compressedDataUrl);
    }
    return uploaded;
}

function compressDatabaseImage(file) {
    return new Promise((resolve, reject) => {
        const reader = new FileReader();
...
function validateDatabaseImagePayload(settings) {
    const approximateBytes = JSON.stringify(settings).length;
    if (approximateBytes > 8 * 1024 * 1024) {
        throw new Error('حجم الصور كبير؛ احفظ 3 إلى 5 صور في كل دفعة.');
    }
}
...
window.saveQuranBenefitsSettings = async () => {
    ...
    try {
        ...
        const reason = error?.code === 'storage/retry-limit-exceeded' || error?.code === 'storage/bucket-not-found'
            ? 'Firebase Storage غير مفعّل أو لا توجد حاوية Storage للمشروع.'
            : (error?.code || error?.message || 'تحقق من تفعيل Firebase Storage وقواعده.');
...
window.saveNamesOfAllahSettings = async () => {
    ...
    try {
        ...
        const uploaded = await uploadImagesToFirebaseStorage(files, 'names-of-allah', status);
        ...
        if (status) status.textContent = `تم رفع وحفظ ${toArabicNum(String(uploaded.length))} صورة لجميع المستخدمين.`;
    } catch (error) { console.error('تعذر حفظ أسماء الله الحسنى:', error); const reason = error?.code === 'storage/retry-limit-exceeded' || error?.code === 'storage/bucket-not-found' ? 'Firebase Storage غير مفعّل أو لا توجد حاوية Storage للمشروع.' : (error?.code || error?.message || 'تحقق من تفعيل Firebase Storage وقواعده.'); if (status) status.textContent = `فشل الحفظ: ${reason}`; }
};
