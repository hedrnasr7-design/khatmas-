...
function renderDateBar() {
    ...
    // التاريخ والنص الإضافي يبقيان داخل نسخة واحدة، ثم تتكرر النسخة مع فاصل صغير.
    const fullText = `${formatted.weekday} / ${formatted.gregorian} / ${formatted.hijri}${extra ? ` • ${extra}` : ''}`;
    const cycleGap = 28;
    track.innerHTML = '';
    [fullText, fullText].forEach(textValue => {
        const copy = document.createElement('span');
    ...
    const copyWidth = Math.max(1, firstCopy?.getBoundingClientRect().width || 1);
    const barWidth = Math.max(1, bar.getBoundingClientRect().width);
    const cycleDistance = copyWidth + cycleGap;
    const effectiveSpeed = Math.max(5, Math.min(120, Number(settings.speed) || 12));
    const duration = Math.max(8, Math.min(180, (barWidth + cycleDistance) / (effectiveSpeed * 10)));
    // تبدأ النسخة الأولى ظاهرة، وتنتهي عند موضع النسخة التالية لضمان تكرار سلس.
    bar.style.setProperty('--date-start', '0px');
    bar.style.setProperty('--date-end', `-${cycleDistance}px`);
    bar.style.setProperty('--date-duration', `${duration}s`);
    if (dateCycleRestartTimer) clearTimeout(dateCycleRestartTimer);
    track.style.animation = 'none';
    track.style.transform = 'translateX(0)';
    void track.offsetWidth;
    track.style.animation = `dateMarquee ${duration}s linear infinite`;
