...
function renderTodayKhatmas() {
    ...
    // المصدر الموحد: نفس قائمة البحث في لوحة التحكم، مع نفس الوقت المخصص
    // ونفس الترتيب. لا نستخدم فلترة يومية منفصلة حتى لا تختلف الخانتان.
    const rows = getScheduleRows('').filter(row => {
        const info = row.info;
        return info.weekdays.length || info.times.length || info.periods.length || getScheduleOverride(row.key);
    });
    ...
    track.style.animationDuration = `${Math.round(300 - (level - 1) * (240 / 99))}s`; track.style.animationIterationCount = 'infinite'; track.style.animationTimingFunction = 'linear'; track.style.animationPlayState = 'running';
}
