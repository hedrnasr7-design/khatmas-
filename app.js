...
window.openTodayKhatma = index => {
    ...
    if (key && typeof window.show === 'function' && allKhatmas?.[key]) {
        Promise.resolve(window.show(key)).catch(error => console.warn('تعذر فتح الختمة من ختمات اليوم:', error));
        return;
    }
    const title = row?.dataset?.khatmaTitle || '';
    const matchedKey = findTodayKhatmaKeyByTitle(title);
    if (matchedKey && typeof window.show === 'function') Promise.resolve(window.show(matchedKey)).catch(error => console.warn('تعذر فتح الختمة من عنوانها:', error));
};
