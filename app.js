...
function explainGeminiRenameError(error) {
        ...
        async function syncDataFromNetwork() {
            ...
            try {
                const canUseOffline = accessMode !== 'limited';
                const previousEtag = canUseOffline ? await OfflineStore.getMeta('firebase-etag') : null;
...
function setAppPage(page) {
    const pages = [null, null, document.getElementById('pageTwo'), document.getElementById('pageThree'), document.getElementById('pageFour')];
    ...
    page = Math.max(1, Math.min(4, Number(page) || 1));
    document.body.classList.remove('app-page-2', 'app-page-3', 'app-page-4');
    if (page > 1 && pages[page]) {
        pages[page].append(controls, tabs);
        document.body.classList.add(`app-page-${page}`);
        const tab = page === 2 ? 'all' : page === 3 ? 'images' : 'fav';
        switchTab(tab);
        renderTodayKhatmas();
    } else {
...
(() => {
    ...
    document.addEventListener('touchend', event => {
        ...
        setAppPage(dx < 0 ? (current === 4 ? 1 : current + 1) : (current === 1 ? 4 : current - 1));
    }, { passive: true });
})();
...
function renderDailyMessage() {
    ...
    let selected;
    if (timed.length) selected = [...timed].reverse().find(item => item.minute <= currentMinute) || timed[timed.length - 1];
    else selected = scheduled[Math.floor(Date.now() / 86400000) % scheduled.length];
...
function renderDateBar() {
    ...
    if (live) {
        live.textContent = `${dateText} ${formatted.gregorian}\n${formatted.hijri} • ${formatIraqClock(iraqNow)}${extra ? `\n${extra}` : ''}`;
