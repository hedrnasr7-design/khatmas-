...
async function saveDailyCardsBulkSettings() {
    ...
    try { const settings = { ...(window.dailyCardsSettings || defaultDailyCardsSettings), days:{...((window.dailyCardsSettings || {}).days || {})}, titleColor:document.getElementById('dailyCardsTitleColor')?.value || '#1b4332', updatedAt:Date.now() }; for (const i of inputs) { const image = await compressDailyMessageImage(i.files[0]); settings.days[i.dataset.bulkKey] = {...(settings.days[i.dataset.bulkKey] || {}), [i.dataset.bulkKind]:image}; } await saveOneBulkSection('appSettings/dailyCards', settings, statusId, inputs.length); localStorage.setItem('daily_cards_settings', JSON.stringify(settings)); window.dailyCardsSettings=settings; applyDailyCardsSettings(settings); inputs.forEach(i=>i.value=''); bulkStatus(statusId, `تم حفظ ${inputs.length} صورة للصلاة والأذكار لجميع المستخدمين.`); } catch (error) { console.error(error); bulkStatus(statusId, `فشل الحفظ: ${error?.code || error?.message || 'تحقق من صلاحيات Firebase.'}`); }
}
async function saveDailyDuaVisitBulkSettings() {
    ...
    try { const settings = { ...(window.dailyDuaVisitSettings || defaultDailyDuaVisitSettings), days:{...((window.dailyDuaVisitSettings || {}).days || {})}, titleColor:document.getElementById('dailyDuaVisitTitleColor')?.value || '#8a4b08', updatedAt:Date.now() }; for (const i of inputs) { const images = await Promise.all([...i.files].map(compressDailyMessageImage)); settings.days[i.dataset.bulkKey] = {...(settings.days[i.dataset.bulkKey] || {}), [i.dataset.bulkKind]:images}; } await saveOneBulkSection('appSettings/dailyDuaVisit', settings, statusId, inputs.length); localStorage.setItem('daily_dua_visit_settings', JSON.stringify(settings)); window.dailyDuaVisitSettings=settings; applyDailyDuaVisitSettings(settings); inputs.forEach(i=>i.value=''); bulkStatus(statusId, `تم حفظ ${inputs.length} خانة للدعاء والزيارة لجميع المستخدمين.`); } catch (error) { console.error(error); bulkStatus(statusId, `فشل الحفظ: ${error?.code || error?.message || 'تحقق من صلاحيات Firebase.'}`); }
}
async function saveNightPrayerBulkSettings() {
    ...
    try { const settings = { ...(window.nightPrayerSettings || defaultNightPrayerSettings), nights:{...((window.nightPrayerSettings || {}).nights || {})}, titleColor:document.getElementById('nightPrayerTitleColor')?.value || '#56308f', updatedAt:Date.now() }; for (const i of inputs) settings.nights[i.dataset.bulkKey] = { images: await Promise.all([...i.files].map(compressDailyMessageImage)) }; await saveOneBulkSection('appSettings/nightPrayer', settings, statusId, inputs.length); localStorage.setItem('night_prayer_settings', JSON.stringify(settings)); window.nightPrayerSettings=settings; applyNightPrayerSettings(settings); inputs.forEach(i=>i.value=''); bulkStatus(statusId, `تم حفظ ${inputs.length} ليلة لجميع المستخدمين.`); } catch (error) { console.error(error); bulkStatus(statusId, `فشل الحفظ: ${error?.code || error?.message || 'تحقق من صلاحيات Firebase.'}`); }
}
...
function applyNightPrayerSettings(settings) {
    ...
    const now = getIraqNow();
    const night = merged.nights[getActiveNightPrayerDay(now)] || {};
    ...
    const dayInput = document.getElementById('nightPrayerDay'); if (dayInput) dayInput.value = String(getActiveNightPrayerDay(now));
