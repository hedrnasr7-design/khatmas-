...
function applyQuotesSettings(settings, saveLocal = true) {
    ...
    if (box && track) {
        ...
        const effectiveSpeed = [22, 50].includes(Number(merged.speed)) ? 12 : (Number(merged.speed) || 12);
        box.style.setProperty('--quotes-speed', `${Math.max(5, Math.min(120, effectiveSpeed))}s`);
    ...
    if (speed) speed.value = [22, 50].includes(Number(merged.speed)) ? 12 : merged.speed;
