...
async function claimAccessCode(code) {
    ...
    const matchedOpen = OPEN_ACCESS_CODES.find(item => item.replace(/[^A-Z0-9]/g, '') === compact);
    const matchedSingle = ACCESS_CODES.find(item => item.replace(/[^A-Z0-9]/g, '') === compact);
    const canonical = matchedOpen || matchedSingle || normalized;
    const isOpen = Boolean(matchedOpen);
    if (!matchedSingle && !matchedOpen) throw new Error('الرمز غير صحيح.');
    const deviceId = getDeviceId();
    const ref = db.ref(`appSettings/accessCodes/${canonical}`);
    let result;
    try {
