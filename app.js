...
async function renderMafatihToday() {
    ...
    try {
        ...
        target.innerHTML = selected.map(item => `<div class="mafatih-today-item"><a href="${escapeHtml(item.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.title)}</a><div>${escapeHtml(item.text.slice(0, 180))}${item.text.length > 180 ? '…' : ''}</div></div>`).join('');
