...
function startAppAfterAccess() {
    ...
    // المستخدمون يرون صفحات الختمات فقط: النصية ثم المصورة ثم المفضلة.
    if (accessMode !== 'admin' && typeof window.setAppPage === 'function') window.setAppPage(2);
    // لا نستخدم التخزين الدائم أو Service Worker؛ التطبيق يعمل عبر الشبكة فقط.
...
async function syncDataFromNetwork() {
            ...
            try {
                const canUseOffline = false;
                const previousEtag = null;
                ...
                console.warn('تعذر تحميل البيانات من الشبكة:', error);
                stopLoadingProgress();
