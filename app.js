	...
	async function syncDataFromNetwork() {
            ...
            try {
                ...
                const parts = await Promise.all(Array.from({ length: 6 }, (_, index) => {
	                ...
	                allKhatmas = removeDeletedKhatmaFromData(await mergeLocalTopicOverrides(merged));
                stopLoadingProgress();
                renderCurrentData();
                try {
                ...
                try {
                    ...
                    if (!Object.keys(data).length) throw new Error('بيانات Firebase فارغة');
	                    allKhatmas = removeDeletedKhatmaFromData(await mergeLocalTopicOverrides(data));
                    stopLoadingProgress();
                    renderCurrentData();
                } catch (error) {
...
window.openTodayKhatma = index => {
    ...
    if (key && typeof window.show === 'function' && allKhatmas?.[key]) {
        window.show(key);
        return;
    }
    const title = row?.dataset?.khatmaTitle || '';
    const matchedKey = findTodayKhatmaKeyByTitle(title);
    if (matchedKey && typeof window.show === 'function') window.show(matchedKey);
};
...
function renderTodayKhatmas() {
    ...
    const manual = (window.todayKhatmasSettings?.manual || []).map(line => {
        ...
        return { label: parts.length >= 2 ? `${parts[0]} (${parts[1]}${parts[2] ? ` — ${parts[2]}` : ''})` : parts[0], key: findTodayKhatmaKeyByTitle(parts[0]), title: parts[0] };
    }).filter(Boolean);
    ...
    const value = overrideText ? overrideText.split(/\n+/).map(v => ({ label: v.trim(), key: findTodayKhatmaKeyByTitle(v), title: v.trim() })).filter(v => v.label) : generatedValue;
    ...
    track.innerHTML = value.map((item, index) => `<button type="button" class="today-khatma-row${isEmpty ? ' today-khatma-empty' : ''}" data-index="${index}" data-khatma-key="${escapeHtml(item.key || '')}" data-khatma-title="${escapeHtml(item.title || '')}" onclick="openTodayKhatma(${index})" ${isEmpty ? 'disabled' : ''}>${escapeHtml(item.label)}</button>`).join('');
    track.style.animation = 'none';
