...
const OPEN_ACCESS_CODES = ['OPEN-FEB5D0E0-05D8', 'OPEN-82AFFCDF-F86E', 'OPEN-3E2D9C6E-1DFE', 'OPEN-8126B963-AA74', 'OPEN-DA9FDB48-2CE8'];
const ACCESS_CODES = ['KHTM-18319104-FF5C', 'KHTM-23101824-C5E9', 'KHTM-57F53891-609E', 'KHTM-0EBF962C-1CF1', 'KHTM-1FEE6061-50F4', 'KHTM-F2CD21EF-FCC5', 'KHTM-5BF3B73B-B8E7', 'KHTM-4B949F2B-6D90', 'KHTM-9AFE9BD6-7060', 'KHTM-1B6E6FF8-202A', 'KHTM-4FA960F1-D0F9', 'KHTM-CD6EDB9E-1697', 'KHTM-7C542A7A-B9AE', 'KHTM-54EF0F12-02F7', 'KHTM-72DCA98A-4DB7', 'KHTM-9739F7FC-9C58', 'KHTM-00F6C6D4-0DB0', 'KHTM-62C30661-E740', 'KHTM-BAC5BCE0-3E93', 'KHTM-54C2FFDD-CCD0'];
const ACCESS_VERSION = 'device-bound-v1';
const ACCESS_STORAGE_KEY = 'khatmas_access_mode';
const ACCESS_CREDENTIAL_KEY = 'khatmas_access_credential';
const DEVICE_ID_KEY = 'khatmas_device_id';
const savedAccessCookie = document.cookie.split('; ').find(item => item.startsWith('khatmas_access_mode='));
let accessMode = '';
...
try { const saved = JSON.parse(localStorage.getItem(ACCESS_CREDENTIAL_KEY) || 'null'); if (saved?.code) { activeAccessCode=saved.code; activeAccessVersion=Number(saved.version)||0; } else { activeAccessCode = localStorage.getItem(ACCESS_CREDENTIAL_KEY) || ''; } accessMode = activeAccessCode ? 'full' : ''; } catch (error) {}
...
async function claimAccessCode(code) {
    ...
    const isOpen = OPEN_ACCESS_CODES.includes(normalized);
    if (!ACCESS_CODES.includes(normalized) && !isOpen) throw new Error('الرمز غير صحيح.');
    const deviceId = getDeviceId();
    const ref = db.ref(`appSettings/accessCodes/${normalized}`);
    let result;
    try {
        result = await ref.transaction(current => {
            if (!current) return { enabled:true, open:isOpen, deviceId:isOpen ? null : deviceId, resetVersion:0, createdAt:firebase.database.ServerValue.TIMESTAMP };
            if (current.enabled === false) return;
            if (!isOpen && current.deviceId && current.deviceId !== deviceId) return;
            return { ...current, open:isOpen, enabled:true, deviceId:isOpen ? null : (current.deviceId || deviceId), claimedAt:current.claimedAt || firebase.database.ServerValue.TIMESTAMP };
        });
    } catch(e) { throw new Error('تعذر التحقق من الكود؛ تحقق من الاتصال بالإنترنت.'); }
    const value=result.snapshot.val();
    if (!result.committed || !value || value.enabled === false) throw new Error('هذا الكود موقوف.');
    if (!value.open && value.deviceId !== deviceId) throw new Error('هذا الكود مستخدم على جهاز آخر.');
    return { code:normalized, version:Number(value.resetVersion)||0 };
}
async function validateAccessCodeOnline(code, version=0) {
    if (!code || !navigator.onLine) return true;
    try { const snap=await db.ref(`appSettings/accessCodes/${code}`).once('value'); const value=snap.val(); if (value && (value.enabled===false || (Number(value.resetVersion)||0)!==Number(version) || (!value.open && value.deviceId && value.deviceId!==getDeviceId()))) return false; } catch(e) {}
    return true;
}
async function validateStoredAccess() {
    if (!activeAccessCode || !navigator.onLine) return true;
    const ok=await validateAccessCodeOnline(activeAccessCode,activeAccessVersion);
    if (!ok) { activeAccessCode=''; activeAccessVersion=0; accessMode=''; try { localStorage.removeItem(ACCESS_CREDENTIAL_KEY); sessionStorage.removeItem(ACCESS_STORAGE_KEY); } catch(e) {} const gate=document.getElementById('accessGate'); if(gate){gate.style.display='flex';gate.style.pointerEvents='auto';} return false; }
    return true;
}
...
function startAppAfterAccess() {
    ...
    if ('serviceWorker' in navigator) navigator.serviceWorker.register(`./sw-v66.js?v=66`, { updateViaCache: 'none' }).catch(() => {});
}
...
window.activateAccessCode = async () => {
    ...
    try {
        ...
        activeAccessCode = claimed.code; activeAccessVersion = claimed.version;
        accessMode='full';
        localStorage.setItem(ACCESS_CREDENTIAL_KEY, JSON.stringify(claimed));
        applyAccessMode('full');
        document.cookie = `khatmas_access_mode=${encodeURIComponent(activeAccessCode)}; max-age=31536000; path=/; SameSite=Lax`;
...
window.loadAccessCodesAdmin = async () => {
    ...
    try { const snap=await db.ref('appSettings/accessCodes').once('value'); const data=snap.val()||{}; box.value=[...ACCESS_CODES,...OPEN_ACCESS_CODES].map(code=>{const v=data[code]||{}; return `${code} | ${v.enabled===false?'موقوف':'فعال'} | ${v.deviceId?'مرتبط بجهاز':'غير مستخدم'}`;}).join('\n'); } catch(e) { box.value='تعذر تحميل حالة الأكواد.'; }
};
window.disableAccessCode = async () => { const code=String(document.getElementById('accessCodeToDisable')?.value||'').trim().toUpperCase(); const status=document.getElementById('accessCodesAdminStatus'); if(!ACCESS_CODES.includes(code) && !OPEN_ACCESS_CODES.includes(code)){if(status)status.textContent='اكتب كودًا صحيحًا.';return;} try { await db.ref(`appSettings/accessCodes/${code}/enabled`).set(false); if(status)status.textContent='تم إيقاف الكود.'; loadAccessCodesAdmin(); } catch(e){if(status)status.textContent='تعذر الإيقاف.';} };
window.enableAccessCode = async () => { const code=String(document.getElementById('accessCodeToDisable')?.value||'').trim().toUpperCase(); const status=document.getElementById('accessCodesAdminStatus'); if(!ACCESS_CODES.includes(code) && !OPEN_ACCESS_CODES.includes(code)){if(status)status.textContent='اكتب كودًا صحيحًا.';return;} try { await db.ref(`appSettings/accessCodes/${code}`).transaction(current => ({...(current||{}), enabled:true, resetVersion:(Number(current?.resetVersion)||0)+1})); if(status)status.textContent='تم تفعيل الكود؛ سيُطلب الكود من المستخدم مرة أخرى.'; loadAccessCodesAdmin(); } catch(e){if(status)status.textContent='تعذر التفعيل.';} };
...
async function syncDataFromNetwork() {
            ...
            try {
                ...
                let response;
                try {
                    response = await fetch('./khatmas-index.json', { cache: 'no-store', signal: controller.signal });
                } catch (indexError) {
                    response = await fetch(`${DATABASE_URL}/khatmas.json`, { method: 'GET', headers, cache: 'no-store', signal: controller.signal });
                }
                ...
                if (newEtag && !response.url.endsWith('khatmas-index.json')) await OfflineStore.setMeta('firebase-etag', newEtag);
...
async function initializeOfflineFirstApp() {
            ...
            const restored = await loadCachedData();
            if (restored) renderCurrentData();
...
window.saveMafatihSettings = async () => { const s = applyMafatihSettings({ font:document.getElementById('mafatihFontSelect')?.value, size:document.getElementById('mafatihFontSize')?.value, bg:document.getElementById('mafatihBgColor')?.value, card:document.getElementById('mafatihCardColor')?.value, accent:document.getElementById('mafatihAccentColor')?.value, text:document.getElementById('mafatihTextColor')?.value, border:document.getElementById('mafatihBorderColor')?.value, header:document.getElementById('mafatihHeaderColor')?.value }, true); try { await db.ref('appSettings/mafatih').set({ ...s, updatedAt: firebase.database.ServerValue.TIMESTAMP }); } catch (error) {} const status = document.getElementById('mafatihSettingsStatus'); if (status) status.textContent = 'تم حفظ إعدادات مفاتيح الجنان لجميع المستخدمين.'; };

function applyMafatihTodaySettings(settings = {}, persist = false) {
    const s = { enabled:true, count:3, bg:'#fffdf0', accent:'#d4af37', text:'#1b4332', border:'#1b4332', font:'Amiri, serif', size:17, customItems:'', ...settings };
    ...
    for (const [id, value] of Object.entries({mafatihTodayEnabled:s.enabled !== false, mafatihTodayCount:s.count, mafatihTodayBg:s.bg, mafatihTodayAccent:s.accent, mafatihTodayText:s.text, mafatihTodayBorder:s.border, mafatihTodayFont:s.font, mafatihTodaySize:s.size, mafatihTodayCustomItems:s.customItems || '', mafatihTodayDisplayText:s.displayOverride || ''})) { const el = document.getElementById(id); if (el) { if (el.type === 'checkbox') el.checked = Boolean(value); else el.value = value; } }
...
async function renderMafatihToday() {
    ...
    try {
        const visibleText = String(mafatihTodaySettings.displayOverride || '').trim();
        if (visibleText) {
            const selected = visibleText.split(/\n+/).map(line => line.trim()).filter(Boolean).map(line => ({ title: line, text: '' }));
            window.__mafatihTodayItems = selected;
            target.innerHTML = selected.map((item,index) => { const full = `${item.title}${item.text ? ` — ${item.text}` : ''}`; const long = full.length > 180; return `<div class="mafatih-today-item"><div>${escapeHtml(long ? full.slice(0,180) + '…' : full)}</div>${long ? `<a href="#" onclick="openMafatihTodayReader(${index});return false;">المزيد</a>` : ''}</div>`; }).join('');
            const editor = document.getElementById('mafatihTodayDisplayText'); if (editor && document.activeElement !== editor) editor.value = visibleText;
            return;
        }
        ...
        window.__mafatihTodayItems = selected;
        target.innerHTML = selected.map((item, index) => { const long = String(item.text || '').length > 180; return `<div class="mafatih-today-item"><strong>${escapeHtml(item.title)}</strong><div>${escapeHtml(String(item.text || '').slice(0,180))}${long ? '…' : ''}</div>${long ? `<a href="#" onclick="openMafatihTodayReader(${index});return false;">المزيد</a>` : ''}</div>`; }).join('');
        const editor = document.getElementById('mafatihTodayDisplayText');
        if (editor && document.activeElement !== editor) editor.value = selected.map(item => `${item.title}${item.text ? ` — ${item.text}` : ''}`).join('\n');
...
let prayerDisplaySettings = { bg:'#fffdf0', text:'#1b4332', accent:'#d4af37', border:'#1b4332', size:17, showSunrise:true };
function applyPrayerDisplaySettings(settings = {}, persist = false) {
    prayerDisplaySettings = { ...prayerDisplaySettings, ...settings };
    const s = prayerDisplaySettings, root = document.documentElement;
    root.style.setProperty('--prayer-bg', s.bg); root.style.setProperty('--prayer-text', s.text); root.style.setProperty('--prayer-accent', s.accent); root.style.setProperty('--prayer-border', s.border); root.style.setProperty('--prayer-size', `${Math.max(12, Math.min(32, Number(s.size) || 17))}px`);
    for (const [id,value] of Object.entries({prayerDisplayBg:s.bg,prayerDisplayText:s.text,prayerDisplayAccent:s.accent,prayerDisplayBorder:s.border,prayerDisplaySize:s.size,prayerDisplayShowSunrise:s.showSunrise})) { const el=document.getElementById(id); if (el) el.type === 'checkbox' ? el.checked = Boolean(value) : el.value = value; }
    if (persist) localStorage.setItem('prayer_display_settings', JSON.stringify(prayerDisplaySettings));
    return prayerDisplaySettings;
}
async function loadPrayerDisplaySettings() { let saved={}; try { saved=JSON.parse(localStorage.getItem('prayer_display_settings')||'{}')||{}; } catch(e) {} try { const snap=await db.ref('appSettings/prayerDisplay').once('value'); if(snap.val()) saved={...saved,...snap.val()}; } catch(e) {} applyPrayerDisplaySettings(saved); }
window.savePrayerDisplaySettings = async () => { const s=applyPrayerDisplaySettings({bg:document.getElementById('prayerDisplayBg')?.value,text:document.getElementById('prayerDisplayText')?.value,accent:document.getElementById('prayerDisplayAccent')?.value,border:document.getElementById('prayerDisplayBorder')?.value,size:document.getElementById('prayerDisplaySize')?.value,showSunrise:document.getElementById('prayerDisplayShowSunrise')?.checked},true); try { await db.ref('appSettings/prayerDisplay').set({...s,updatedAt:firebase.database.ServerValue.TIMESTAMP}); } catch(e) {} const el=document.getElementById('prayerDisplayStatus'); if(el) el.textContent='تم حفظ مظهر مواقيت الصلاة لجميع المستخدمين.'; renderHomePrayerTimes(); };
...
async function renderHomePrayerTimes() {
    ...
    const toArabic12 = value => {
        ...
        hour = hour % 12 || 12;
        return `${toArabicNum(String(hour))}:${toArabicNum(minute)} ${suffix}`;
    };
    status.textContent = 'جارٍ تحديد موقعك لحساب المواقيت...';
    navigator.geolocation.getCurrentPosition(async position => {
        try {
            ...
            try { localStorage.setItem('last_prayer_times', JSON.stringify({ times, location: data?.data?.meta?.timezone || `${latitude.toFixed(2)}, ${longitude.toFixed(2)}`, savedAt: Date.now() })); } catch(e) {}
            const location = data?.data?.meta?.timezone || `${latitude.toFixed(2)}, ${longitude.toFixed(2)}`;
            const labels = [['Fajr','الفجر','🌙'], ...(prayerDisplaySettings.showSunrise === false ? [] : [['Sunrise','الشروق','🌅']]), ['Dhuhr','الظهر','☀️'],['Asr','العصر','🌤️'],['Maghrib','المغرب','🌇'],['Isha','العشاء','🌃']];
            list.innerHTML = `<div class="prayer-grid">${labels.map(([key, label, icon]) => `<div class="prayer-time-item"><div class="prayer-icon" aria-hidden="true">${icon}</div><strong>${label}</strong><span dir="rtl">${escapeHtml(toArabic12(times[key]))}</span></div>`).join('')}</div>`;
            status.textContent = `مواقيت الصلاة حسب موقعك — ${escapeHtml(location)}`;
        } catch (error) {
            console.error('تعذر تحميل مواقيت الصلاة:', error);
            try {
                const cached = JSON.parse(localStorage.getItem('last_prayer_times') || 'null');
                if (cached?.times) {
                    const labels = [['Fajr','الفجر','🌙'], ...(prayerDisplaySettings.showSunrise === false ? [] : [['Sunrise','الشروق','🌅']]), ['Dhuhr','الظهر','☀️'],['Asr','العصر','🌤️'],['Maghrib','المغرب','🌇'],['Isha','العشاء','🌃']];
                    list.innerHTML = `<div class="prayer-grid">${labels.map(([key,label,icon]) => `<div class="prayer-time-item"><div class="prayer-icon">${icon}</div><strong>${label}</strong><span dir="rtl">${escapeHtml(toArabic12(cached.times[key]))}</span></div>`).join('')}</div>`;
                    status.textContent = `مواقيت محفوظة للعمل دون إنترنت — ${escapeHtml(cached.location || '')}`;
                    return;
                }
            } catch(e) {}
            status.textContent = 'لا توجد مواقيت محفوظة؛ افتح التطبيق مرة واحدة مع الإنترنت.';
            list.innerHTML = '';
        }
    }, error => {
        console.warn('لم يتم السماح بتحديد الموقع:', error);
        status.textContent = 'اسمح بتحديد موقعك لعرض المواقيت حسب مدينتك.';
        list.innerHTML = '';
    }, { enableHighAccuracy: false, timeout: 12000, maximumAge: 300000 });
}

function arrangeHomeSections() { const today = document.getElementById('mafatihTodayBox'), prayer = document.getElementById('mafatihPrayerBox'); if (today) today.style.setProperty('display', 'block', 'important'); if (prayer) prayer.style.setProperty('display', 'block', 'important'); mafatihTodaySettings.enabled = true; renderMafatihToday(); renderHomePrayerTimes(); }
async function loadMafatihTodaySettings() { let saved = {}; try { saved = JSON.parse(localStorage.getItem('mafatih_today_settings') || '{}') || {}; } catch (error) {} try { const snapshot = await db.ref('appSettings/mafatihToday').once('value'); if (snapshot.val()) saved = { ...saved, ...snapshot.val() }; } catch (error) {} mafatihTodaySettings = applyMafatihTodaySettings(saved); localStorage.setItem('mafatih_today_settings', JSON.stringify(mafatihTodaySettings)); renderMafatihToday(); }
window.saveMafatihTodaySettings = async () => { const s = applyMafatihTodaySettings({enabled:document.getElementById('mafatihTodayEnabled')?.checked, count:document.getElementById('mafatihTodayCount')?.value, bg:document.getElementById('mafatihTodayBg')?.value, accent:document.getElementById('mafatihTodayAccent')?.value, text:document.getElementById('mafatihTodayText')?.value, border:document.getElementById('mafatihTodayBorder')?.value, font:document.getElementById('mafatihTodayFont')?.value, size:document.getElementById('mafatihTodaySize')?.value, customItems:document.getElementById('mafatihTodayCustomItems')?.value || '', displayOverride:document.getElementById('mafatihTodayDisplayText')?.value || ''}, true); mafatihTodaySettings = s; try { await db.ref('appSettings/mafatihToday').set({ ...s, updatedAt: firebase.database.ServerValue.TIMESTAMP }); } catch (error) {} await renderMafatihToday(); const status = document.getElementById('mafatihTodaySettingsStatus'); if (status) status.textContent = 'تم حفظ إعدادات أعمال اليوم لجميع المستخدمين.'; };
...
function setAppPage(page) {
    ...
    const prayer = document.getElementById('mafatihPrayerBox');
    if (prayer) prayer.style.setProperty('display', page === 1 ? 'block' : 'none', 'important');
    if (page > 1 && pages[page]) {
        pages[page].append(controls, tabs, topicTabs);
...
function renderTodayKhatmas() {
    ...
    const labels = rows.map(row => {
        ...
        return `${info.number ? toArabicNum(info.number) + ' — ' : ''}${getScheduleDisplayTitle(info)}${info.hasImage ? ' — ختمة مصورة' : ''} (${time})`;
    }).slice(0, 120);
    const manual = (window.todayKhatmasSettings?.manual || []).map(line => {
        const parts = String(line).split('|').map(v => v.trim());
        return parts.length >= 2 ? `${parts[0]} (${parts[1]}${parts[2] ? ` — ${parts[2]}` : ''})` : parts[0];
    }).filter(Boolean);
    const generatedValue = [...labels, ...manual].length ? [...labels, ...manual] : ['لا توجد ختمات حاليا'];
    const overrideText = String(window.todayKhatmasSettings?.displayOverride || '').trim();
    const value = overrideText ? overrideText.split(/\n+/).map(v => v.trim()).filter(Boolean) : generatedValue;
    const displayInput = document.getElementById('todayKhatmasDisplayText'); if (displayInput && document.activeElement !== displayInput) displayInput.value = value.join('\n');
    // خانة ثابتة جديدة: لا حركة ولا transform ولا دورة زمنية، حتى يبقى النص ظاهرًا دائمًا.
    const isEmpty = value.length === 1 && value[0] === 'لا توجد ختمات حاليا';
    track.innerHTML = value.map((label, index) => `<div class="today-khatma-row${isEmpty ? ' today-khatma-empty' : ''}" data-index="${index}">${escapeHtml(label)}</div>`).join('');
    track.style.animation = 'none';
...
window.saveTodayKhatmasSettings = async () => { try { await requireAdminSession(); } catch (error) { document.getElementById('todayKhatmasStatus').textContent = 'انتهت جلسة المشرف؛ سجّل الدخول من جديد ثم حاول.'; return; } const settings = { text: document.getElementById('todayKhatmasTextInput')?.value.trim() || '', displayOverride: document.getElementById('todayKhatmasDisplayText')?.value || '', excluded: String(document.getElementById('todayKhatmasExcluded')?.value || '').split(/\n+/).map(v => v.trim()).filter(Boolean), manual: String(document.getElementById('todayKhatmasManual')?.value || '').split(/\n+/).map(v => v.trim()).filter(Boolean), bg: document.getElementById('todayKhatmasBgColor')?.value || '#fffdf0', textColor: document.getElementById('todayKhatmasTextColor')?.value || '#1b4332', border: document.getElementById('todayKhatmasBorderColor')?.value || '#1b4332', size: Number(document.getElementById('todayKhatmasSize')?.value) || 18, motionLevel: Number(document.getElementById('todayKhatmasMotionLevel')?.value) || 50 };
    ...
    catch (error) { console.error(error); document.getElementById('todayKhatmasStatus').textContent = `تعذر الحفظ: ${error.code || error.message || 'تحقق من قواعد Firebase.'}`; }
};
