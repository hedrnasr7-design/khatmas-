...
const ACCESS_CODES = { limited: '0B115766', full: '4E155583' };
...
function startAppAfterAccess() {
    ...
    if (accessMode !== 'limited' && 'storage' in navigator && 'persist' in navigator.storage) navigator.storage.persist().catch(() => {});
    if (accessMode !== 'limited' && 'serviceWorker' in navigator) {
...
window.activateAccessCode = () => {
    ...
    const mode = value === ACCESS_CODES.limited ? 'limited' : value === ACCESS_CODES.full ? 'full' : '';
...
function explainGeminiRenameError(error) {
        ...
        async function syncDataFromNetwork() {
            ...
            try {
                const previousEtag = accessMode === 'limited' ? null : await OfflineStore.getMeta('firebase-etag');
                ...
                if (accessMode !== 'limited') await OfflineStore.replaceAllKhatmas(data);
...
async function initializeOfflineFirstApp() {
            ...
            if (accessMode !== 'limited') await loadCachedData();
...
function loadLocalQuotesSettings() {
    if (accessMode === 'limited') return defaultQuotesSettings;
...
function getArabicPeriodInfo(source) {
    ...
    if (/(?:بعد\s+)?صلاة\s+(?:الفجر|الصبح)|وقت\s+الصبح|الصباح/.test(text)) add('بعد صلاة الصبح', 5 * 60, 10 * 60, days);
    if (/(?:بعد\s+)?صلاة\s+(?:الظهر|الظهرين)|وقت\s+الظهر/.test(text)) add('بعد صلاة الظهر', 12 * 60, 16 * 60, days);
    if (/(?:عصر|بعد\s+صلاة\s+العصر)|وقت\s+العصر/.test(text)) add('العصر', 15 * 60, 19 * 60, days);
    if (/(?:بعد\s+صلاة\s+المغرب|المغرب|بعد\s+الغروب|المساء)/.test(text)) add('بعد المغرب', 18 * 60, 24 * 60, days);
...
function renderScheduleResults(rows) {
    ...
    box.innerHTML = `<table class="date-schedule-table"><thead><tr><th>الرقم</th><th>العنوان</th><th>اليوم المستخرج</th><th>الوقت المستخرج</th><th>وقت مخصص</th><th>النوع</th></tr></thead><tbody>${rows.map(row => {
        ...
        const times = [...(override ? [override.text] : i.times.map(time => time.text)), ...i.periods.map(period => period.label), ...i.timePhrases].filter((value, index, list) => list.indexOf(value) === index).join('، ') || 'غير محدد';
        const inputId = `schedule-time-${encodeURIComponent(row.key)}`;
        return `<tr><td>${i.number ? toArabicNum(i.number) : '—'}</td><td>${escapeHtml(i.title)}<div class="schedule-muted">${escapeHtml(i.content.slice(0, 180))}${i.content.length > 180 ? '…' : ''}</div></td><td>${escapeHtml(days)}</td><td>${escapeHtml(times)}</td><td><input id="${inputId}" type="text" inputmode="numeric" placeholder="18:30" value="${override ? `${String(override.hour).padStart(2, '0')}:${String(override.minute).padStart(2, '0')}` : ''}" style="width:90px; padding:5px; box-sizing:border-box; direction:ltr;"><button type="button" class="action-btn" onclick="saveKhatmaScheduleTime('${escapeHtml(row.key)}')">حفظ</button></td><td>${i.hasImage ? 'مصورة' : 'نصية'}</td></tr>`;
    }).join('')}</tbody></table>`;
...
window.saveKhatmaScheduleTime = async key => {
    ...
    if (raw && !normalizeScheduleTime(raw)) {
        if (status) status.textContent = 'اكتب الوقت بصيغة 24 ساعة مثل 18:30.';
        return;
    }
    try {
...
function renderDateBar() {
    ...
    const scheduleTitle = scheduled ? getScheduleDisplayTitle(scheduled.row.info) : '';
