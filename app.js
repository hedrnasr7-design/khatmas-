...
async function syncDataFromNetwork() {
            ...
            try {
                ...
                const networkTimeout = setTimeout(() => controller.abort(), 120000);
...
const mafatihSections = [
    { id:'daily', title:'أعمال اليوم', text:'قسم مخصص لإضافة أعمال اليوم والأدعية اليومية.' },
    { id:'months', title:'أعمال الشهور', text:'قسم مخصص لإضافة أعمال الشهور والمناسبات.' },
    { id:'duas', title:'الأدعية', text:'قسم الأدعية. يمكن إضافة النصوص المرخّصة من مصدرها هنا.' },
    { id:'ziyarat', title:'الزيارات', text:'قسم الزيارات. يمكن إضافة النصوص المرخّصة من مصدرها هنا.' },
    { id:'salat', title:'الصلوات', text:'قسم الصلوات والأعمال الخاصة.' },
    { id:'bakiyat', title:'الباقيات الصالحات', text:'قسم الباقيات الصالحات.' }
];
let mafatihCatalog = mafatihSections;
let mafatihContentLoaded = false;
async function loadMafatihContent() {
    if (mafatihContentLoaded) return;
    try {
        let data = null;
        try { data = JSON.parse(localStorage.getItem('mafatih_content_v1') || 'null'); } catch (error) {}
        if (!data) {
            const response = await fetch('./mafatih-content.json', { cache: 'no-store' });
            if (!response.ok) throw new Error('content fetch failed');
            data = await response.json();
            try { localStorage.setItem('mafatih_content_v1', JSON.stringify(data)); } catch (error) {}
        }
        if (Array.isArray(data.pages) && data.pages.length) {
            mafatihCatalog = data.pages.map(page => ({ id:page.id, title:`صفحة ${page.page} — ${page.title}`, text:page.text, page:page.page }));
        }
        mafatihContentLoaded = true;
        renderMafatihSections();
    } catch (error) { console.warn('تعذر تحميل نص مفاتيح الجنان:', error); }
}
function renderMafatihSections(list = mafatihCatalog.slice(0, 120)) {
    const box = document.getElementById('mafatihSections'); if (!box) return;
    box.innerHTML = list.map(item => `<button class="mafatih-section" type="button" onclick="openMafatihReader('${item.id}')">${escapeHtml(item.title)}</button>`).join('');
}
window.openMafatihPage = () => { setAppPage(5); renderMafatihSections(); loadMafatihContent(); };
window.filterMafatihSections = () => {
    const q = String(document.getElementById('mafatihSearch')?.value || '').trim().toLowerCase();
    renderMafatihSections(mafatihCatalog.filter(item => !q || item.title.toLowerCase().includes(q) || item.text.toLowerCase().includes(q)).slice(0, 120));
};
window.openMafatihReader = id => {
    const item = mafatihCatalog.find(section => section.id === id); const reader = document.getElementById('mafatihReader'); if (!item || !reader) return;
    const favorites = JSON.parse(localStorage.getItem('mafatih_favorites') || '[]'); const saved = favorites.includes(id);
    reader.style.display = 'block'; reader.innerHTML = `<strong>${escapeHtml(item.title)}</strong><br><br>${escapeHtml(item.text)}<br><br><button class="mafatih-action" type="button" onclick="toggleMafatihFavorite('${id}')">${saved ? 'إزالة من المفضلة' : 'إضافة إلى المفضلة'}</button>`;
    reader.scrollIntoView({ behavior:'smooth', block:'start' });
};
window.toggleMafatihFavorite = id => { let items = JSON.parse(localStorage.getItem('mafatih_favorites') || '[]'); items = items.includes(id) ? items.filter(value => value !== id) : [...items, id]; localStorage.setItem('mafatih_favorites', JSON.stringify(items)); openMafatihReader(id); };
window.showMafatihSections = () => { const reader = document.getElementById('mafatihReader'); if (reader) reader.style.display = 'none'; renderMafatihSections(mafatihCatalog.slice(0, 120)); };
window.showMafatihFavorites = () => { const ids = JSON.parse(localStorage.getItem('mafatih_favorites') || '[]'); renderMafatihSections(mafatihCatalog.filter(item => ids.includes(item.id))); };
window.showPrayerTimes = () => { const reader = document.getElementById('mafatihReader'); if (reader) { reader.style.display = 'block'; reader.textContent = 'أوقات الصلاة والقبلة يمكن تفعيلها لاحقًا حسب موقع المستخدم وإعداداته.'; } };
(() => {
