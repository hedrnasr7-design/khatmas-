...
// لا ترسم أقسام الجدولة قبل اكتمال تنفيذ الملف وتعريف متغيراته.
let appRuntimeReady = false;
// تُعرّف مبكرًا لأن التطبيق قد يستعيد البيانات المحلية قبل الوصول إلى
// قسم الجدولة في نهاية الملف.
...
function startAppAfterAccess() {
    ...
    if ('serviceWorker' in navigator) navigator.serviceWorker.register(`./sw-v66.js?v=76`, { updateViaCache: 'none' }).catch(() => {});
}
...
function explainGeminiRenameError(error) {
        ...
        function renderCurrentData() {
            ...
            // تُعرّف متغيرات الجدولة في أسفل الملف؛ نؤجل هذا الرسم إلى
            // نهاية دورة التنفيذ حتى لا يحدث Temporal Dead Zone عند الفتح.
            setTimeout(() => {
                try { renderTodayKhatmas(); } catch (error) { console.warn('تعذر رسم ختمات اليوم مؤقتًا:', error); }
            }, 0);
        }
...
// إعدادات «أعمال اليوم» — هذه الدوال يجب أن تكون موجودة قبل استدعائها
// في بداية التطبيق، وإلا يتوقف تنفيذ app.js وتبقى الخانة على «جارٍ التحميل».
let mafatihTodaySettings = {
    enabled: true, count: 3, bg: '#fffdf0', accent: '#d4af37', text: '#1b4332',
    border: '#1b4332', font: 'Amiri, serif', size: 17, customItems: '', displayOverride: ''
};

function applyMafatihTodaySettings(settings = {}, persist = false) {
    mafatihTodaySettings = { ...mafatihTodaySettings, ...settings };
    const s = mafatihTodaySettings;
    const box = document.getElementById('mafatihTodayBox');
    const content = document.getElementById('mafatihTodayContent');
    if (box) {
        box.style.setProperty('--mafatih-today-bg', s.bg || '#fffdf0');
        box.style.setProperty('--mafatih-today-accent', s.accent || '#d4af37');
        box.style.setProperty('--mafatih-today-text', s.text || '#1b4332');
        box.style.setProperty('--mafatih-today-border', s.border || '#1b4332');
        box.style.fontFamily = s.font || 'Amiri, serif';
        box.style.fontSize = `${Math.max(12, Math.min(32, Number(s.size) || 17))}px`;
        box.style.display = s.enabled === false ? 'none' : '';
    }
    if (content) content.style.fontFamily = s.font || 'Amiri, serif';
    ['Enabled','Count','Bg','Accent','Text','Border','Font','Size','CustomItems','DisplayText'].forEach(key => {
        const el = document.getElementById(`mafatihToday${key}`);
        if (!el) return;
        const value = key === 'Enabled' ? s.enabled !== false : key === 'Count' ? Number(s.count) || 3 : key === 'Size' ? Number(s.size) || 17 : key === 'CustomItems' ? (s.customItems || '') : key === 'DisplayText' ? (s.displayOverride || '') : s[key.toLowerCase()];
        if (key === 'Enabled') el.checked = value; else if (value != null) el.value = value;
    });
...
async function renderMafatihToday() {
    ...
    if (!content) return;
    const s = mafatihTodaySettings;
    if (s.enabled === false) { content.innerHTML = ''; return; }
    const custom = String(s.displayOverride || '').trim();
    let items = [];
    if (custom) items = custom.split(/\n+/).map(text => ({ title: 'عمل اليوم', text: text.trim() })).filter(x => x.text);
    if (!items.length && String(s.customItems || '').trim()) {
        items = String(s.customItems).split(/\n+/).map(line => {
            const [title, ...rest] = line.split('|'); return { title: (title || 'عمل اليوم').trim(), text: rest.join('|').trim() };
        }).filter(x => x.text);
    }
    if (!items.length) {
        try {
            ...
            items = pages.filter(x => x && (x.title || x.text)).slice(0, Math.max(1, Number(s.count) || 3)).map(x => ({ title: x.title || 'عمل اليوم', text: x.text || '' }));
        } catch (error) {}
    }
    content.innerHTML = items.length ? items.map((item, index) => `<button type="button" class="mafatih-today-item" data-today-index="${index}"><strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.text)}</span></button>`).join('') : '<div class="mafatih-status">لا توجد أعمال متاحة حاليًا.</div>';
}

async function loadMafatihTodaySettings() {
    let saved = {};
    try { saved = JSON.parse(localStorage.getItem('mafatih_today_settings') || '{}') || {}; } catch (error) {}
    applyMafatihTodaySettings(saved);
    try {
        const snapshot = await db.ref('appSettings/mafatihToday').once('value');
        if (snapshot.val()) applyMafatihTodaySettings({ ...saved, ...snapshot.val() }, true);
    } catch (error) {}
    await renderMafatihToday();
}

window.saveMafatihTodaySettings = async () => {
    const status = document.getElementById('mafatihTodaySettingsStatus');
    const s = { ...applyMafatihTodaySettings({enabled:document.getElementById('mafatihTodayEnabled')?.checked, count:document.getElementById('mafatihTodayCount')?.value, bg:document.getElementById('mafatihTodayBg')?.value, accent:document.getElementById('mafatihTodayAccent')?.value, text:document.getElementById('mafatihTodayText')?.value, border:document.getElementById('mafatihTodayBorder')?.value, font:document.getElementById('mafatihTodayFont')?.value, size:document.getElementById('mafatihTodaySize')?.value, customItems:document.getElementById('mafatihTodayCustomItems')?.value || '', displayOverride:document.getElementById('mafatihTodayDisplayText')?.value || ''}, true), updatedAt:Date.now() };
    mafatihTodaySettings = s;
    localStorage.setItem('mafatih_today_settings', JSON.stringify(s));
    if (status) status.textContent = 'جارٍ حفظ التعديلات لجميع المستخدمين...';
    try {
        if (!firebase.auth().currentUser) throw new Error('يجب تسجيل دخول المشرف أولًا.');
        const ref = db.ref('appSettings/mafatihToday');
        await ref.set({ ...s, updatedAt: firebase.database.ServerValue.TIMESTAMP });
        const check = await ref.once('value');
        if (!check.exists()) throw new Error('لم يتم تأكيد الحفظ في Firebase.');
        await renderMafatihToday();
        if (status) status.textContent = 'تم حفظ النص والألوان لجميع المستخدمين.';
    } catch (error) {
        console.error('فشل حفظ إعدادات أعمال اليوم:', error);
...
function getIraqNow() {
    // استخدم المنطقة مباشرة حتى لا تتوقف التهيئة إذا استُدعيت الدالة
    // من البيانات المحلية قبل تنفيذ قسم إعدادات الساعة في الملف.
    const parts = new Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Baghdad', year:'numeric', month:'numeric', day:'numeric', hour:'numeric', minute:'numeric', second:'numeric', hour12:false, weekday:'short' }).formatToParts(new Date());
...
function arrangeHomeSections() {
    ...
    if (!sticky) return;
    // أعد عناصر البحث والتبويبات إلى الصفحة الرئيسية بعد الرجوع من أي صفحة.
    [controls, tabs, topicTabs].forEach(element => {
        if (element && element.parentElement !== sticky) sticky.appendChild(element);
    });
    const list = document.getElementById('listContainer');
...
// أصبحت جميع متغيرات الجدولة والتاريخ معرفة الآن.
